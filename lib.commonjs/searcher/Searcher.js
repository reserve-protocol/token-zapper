 ...
 */
const findPrecursorTokenSet = async (universe, userInputQuantity, rToken, unitBasket) => {
...
class Searcher {
    ...
    async findSingleInputToBasketGivenBasketUnit(inputQuantity, rToken, basketUnit, slippage) {
         ...
         */
        const precursorTokens = await (0, exports.findPrecursorTokenSet)(this.universe, inputQuantity, rToken, basketUnit);
        /**
    ...
    async findSingleInputToRTokenZap(userInput, rToken, signerAddress, slippage = 0.0) {
        ...
        const searcherResult = new SearcherResult_1.SearcherResult(this.universe, new Swap_1.SwapPaths(this.universe, [userInput], [...inputQuantityToBasketTokens.swapPaths, rTokenMint], output, rTokenMint.outputValue, signerAddress), signerAddress, rToken);
    ...
    async internalQuoter(input, output, destination, slippage = 0.0, maxHops = 2) {
        ...
        const swapPlans = bfsResult.steps
            ...
            if (plan.steps.filter((step) => step instanceof Curve_1.CurveSwap).length > 1) {
                return false;
            }
