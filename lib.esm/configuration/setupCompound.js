...
export async function setupCompoundLike(universe, deployment, cTokens) {
    ...
    for (const { wrappedToken, underlying, collaterals } of cTokens) {
        ...
        for (const collateral of collaterals ?? []) {
            universe.defineMintable(new MintCTokenWrapperAction(universe, wrappedToken, collateral, async () => ONE), new BurnCTokenWrapperAction(universe, wrappedToken, collateral, async () => ONE));
        }
    }
    return cTokens;
}
