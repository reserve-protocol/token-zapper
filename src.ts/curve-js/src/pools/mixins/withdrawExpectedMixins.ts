...
import {
  _calcExpectedAmounts,
  _calcExpectedUnderlyingAmountsMeta,
} from './common'
...
export const withdrawExpectedMixin: PoolTemplate = {
  async withdrawExpected(lpTokenAmount: number | string): Promise<string[]> {
    ...
    return _expected.map((amount, i: number) =>
      formatUnits(amount, this.underlyingDecimals[i])
    )
  },
}
...
export const withdrawExpectedLendingOrCryptoMixin: PoolTemplate = {
  async withdrawExpected(lpTokenAmount: number | string): Promise<string[]> {
    const _lpTokenAmount = parseUnits(lpTokenAmount)
    const _expectedAmounts = await _calcExpectedAmounts.call(
      this,
      _lpTokenAmount
    )
    // @ts-ignore
    const _rates: bigint[] = await this._getRates()
    const _expected = _expectedAmounts.map(
      (_amount, i: number) =>
        (_amount.toBigInt() * BigInt(_rates[i])) / 10n ** 18n
    )
...
export const withdrawExpectedMetaMixin: PoolTemplate = {
  async withdrawExpected(lpTokenAmount: number | string): Promise<string[]> {
    const _lpTokenAmount = parseUnits(lpTokenAmount)
    const _expected = await _calcExpectedUnderlyingAmountsMeta.call(
      this,
      _lpTokenAmount
    )
...
export const withdrawWrappedExpectedMixin: PoolTemplate = {
  async withdrawWrappedExpected(
    ...
    return _expected.map((amount, i: number) =>
      formatUnits(amount, this.wrappedDecimals[i])
    )
  },
}
