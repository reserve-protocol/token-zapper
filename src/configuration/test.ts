...
const initialize = async (universe: Universe) => {
  ...
  const cUSDT = universe.createToken(Address.fromHexString('0xf650c3d88d12db855b8bf7d11be6c55a4e07dcc9'), 'cUSDT', 'cUSDT', 6)
  ...
  const basketHandler = {
    async getMintQuantities() { return await Promise.resolve(quantities) },
    rToken: eUSD,
    inputTokens: quantities
  } as any
  ...
  for (const saToken of saTokens) {
    const rate = 1110924415157506442300940896n
    universe.defineMintable(
