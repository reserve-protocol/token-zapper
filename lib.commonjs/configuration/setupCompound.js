...
async function setupCompoundLike(universe, deployment, cTokens) {
    ...
    for (const { wrappedToken, underlying, collaterals } of cTokens) {
        ...
        for (const collateral of collaterals ?? []) {
            await universe.defineMintable(new CTokenWrapper_1.MintCTokenWrapperAction(universe, wrappedToken, collateral, async () => ONE), new CTokenWrapper_1.BurnCTokenWrapperAction(universe, wrappedToken, collateral, async () => ONE));
        }
    }
    return cTokens;
}
