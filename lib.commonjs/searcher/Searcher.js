...
class Searcher {
    ...
    async findSingleInputToBasketGivenBasketUnit(inputQuantity, rToken, basketUnit, internalTradeSlippage, onResult, abortSignal) {
        ...
        const allOptions = await this.perf.measurePromise('generateAllPermutations', (0, MultiChoicePath_1.generateAllPermutations)(this.universe, multiTrades, precursorSet), rToken.symbol);
        ...
        for (const candidates of (0, MultiChoicePath_1.chunkifyIterable)(allOptions, this.maxConcurrency, abortSignal)) {
    ...
    async redeem(rTokenQuantity, output, signerAddress, opts) {
        ...
        const controller = (0, MultiChoicePath_1.createConcurrentStreamingSeacher)(this, toTxArgs);
        void Promise.race([
    ...
    async findRTokenIntoSingleTokenZapViaRedeem__(rTokenQuantity, output, signerAddress, slippage, onResult, abortSignal, startTime) {
        ...
        const generatePermutation = async (underlyingToOutputTrades) => {
            ...
            const zap = new SearcherResult_1.RedeemZap(this.universe, rTokenQuantity, {
                full: outputSwap,
        ...
        if (permutableTrades.length === 0) {
            ...
            const allposibilities = await (0, MultiChoicePath_1.generateAllPermutations)(this.universe, permutableTrades, new Set([outputToken]));
    ...
    async findTokenZapViaTrade(userInput, rToken, signerAddress, slippage, onResult, abortSignal, startTime = Date.now()) {
        ...
        let results = 0;
        this.findSingleInputTokenSwap_(inputTokenQuantity, rToken, signerAddress, slippage, ownController.signal, 3, false, async (path) => {
            if (results >= 2) {
                return;
            }
            await onResult(new SearcherResult_1.ZapViaATrade(this.universe, userInput, path.intoSwapPaths(this.universe), signerAddress, rToken, startTime, abortSignal));
            results += 1;
            if (results >= 2) {
                ownController.abort();
            }
    ...
    async zapIntoRToken(userInput, rToken, userAddress, opts) {
        ...
        this.checkIfSimulationSupported();
        const controller = (0, MultiChoicePath_1.createConcurrentStreamingSeacher)(this, toTxArgs);
        void this.findSingleInputToRTokenZap_(userInput, rToken, userAddress, slippage, controller.onResult, controller.abortController.signal, start).catch((e) => { });
        void this.findTokenZapViaTrade(userInput, rToken, userAddress, slippage, controller.onResult, controller.abortController.signal, start).catch(() => { });
        await controller.resultReadyPromise;
        return controller.getResults(start);
    }
    ...
    async findSingleInputTokenSwap_(input, output, destination, slippage, abort, maxHops = 2, dynamicInput = false, onResult, rejectRatio = 0.95) {
