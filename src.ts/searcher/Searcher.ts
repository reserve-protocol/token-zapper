...
export class Searcher<SearcherUniverse extends Universe<Config>> {
  ...
  constructor(public readonly universe: SearcherUniverse) {
    ...
    this.loggers = {
      searching: universe.logger.child({
        name: 'searcher',
        phase: 'searching',
      }),
      txGen: universe.logger.child({ name: 'searcher', phase: 'tx-gen' }),
      simulation: universe.logger.child({
        name: 'searcher',
        phase: 'simulation',
      }),
    }
      ...
      async ({
        ...
        const swapPlans = allPlans
          .filter((plan) => {
            ...
            if (
              new Set(plan.steps.map((i) => i.address)).size !==
              plan.steps.length
            ) {
              return false
            }
  ...
  public async zapIntoYieldPosition(
    userInput: TokenQuantity,
    inputToken: Token,
    yieldPosition: Token,
    userAddress: Address,
    ...
    return await this.zapIntoRToken(userInput, inputToken, userAddress, {
      ...opts,
      enableTradeZaps: false,
      endPosition: yieldPosition,
    })
