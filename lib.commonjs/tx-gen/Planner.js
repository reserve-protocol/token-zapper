...
exports.printPlan = exports.StatefullPlanner = exports.Planner = exports.CommandType = exports.Contract = exports.encodeArg = exports.FunctionCall = exports.CommandFlags = exports.SubplanValue = exports.StateValue = exports.ReturnValue = exports.LiteralValue = void 0;
...
let valueId = 0;
const valueNames = new DefaultMap_1.DefaultMap(() => {
...
class StateValue {
    param;
    constructor() {
        this.param = abi_1.ParamType.from('bytes[]');
    }
}
...
exports.SubplanValue = SubplanValue;
/**
 * CommandFlags
...
class FunctionCall {
    ...
    callvalue;
    /** @hidden */
    constructor(contract, flags, fragment, args, callvalue) {
    ...
    withValue(value) {
        ...
        return new FunctionCall(this.contract, (this.flags & ~CommandFlags.CALLTYPE_MASK) | CommandFlags.CALL_WITH_VALUE, this.fragment, this.args, encodeArg(value, abi_1.ParamType.from('uint')));
    }
    /**
...
function abiEncodeSingle(param, value) {
    if (isDynamicType(param)) {
        return new LiteralValue(param, (0, bytes_1.hexDataSlice)(abi_1.defaultAbiCoder.encode([param], [value]), 32));
    }
    return new LiteralValue(param, abi_1.defaultAbiCoder.encode([param], [value]));
}
...
class BaseContract {
    ...
    constructor(address, contractInterface, commandflags = 0) {
        this.interface = (0, properties_1.getStatic)(new.target, 'getInterface')(contractInterface);
        ...
        Object.keys(this.interface.functions).forEach((signature) => {
            ...
            if (this[signature] == null) {
                (0, properties_1.defineReadOnly)(this, signature, buildCall(this, fragment));
            }
            // We do not collapse simple calls on this bucket, which allows
            // frameworks to safely use this without introspection as well as
            // allows decoding error recovery.
            if (this.functions[signature] == null) {
                (0, properties_1.defineReadOnly)(this.functions, signature, buildCall(this, fragment));
            }
        });
...
class Planner {
    ...
    add(call, comment, variableName) {
        ...
        if (call.flags & CommandFlags.TUPLE_RETURN) {
            const ret = new ReturnValue(abi_1.ParamType.fromString('bytes'), command, variableName);
    ...
    buildCommands(ps) {
        ...
        for (let command of this.commands) {
            if (command.type === CommandType.SUBPLAN) {
                ...
                ps.state.push((0, bytes_1.hexDataSlice)(abi_1.defaultAbiCoder.encode(['bytes32[]'], [subcommands]), 32));
...
const makeTokenUsersTracker = () => new DefaultMap_1.DefaultMap((token) => {
    const state = {
        users: [],
        total: 0,
    };
    const fractionFor = (user) => {
        const userStats = state.users.length === 0
            ? undefined
        ...
        return {
            fraction,
            quantity: token.from(BigInt((fraction * total).toFixed(token.decimals))),
        };
    };
    return {
        get total() {
        ...
        addUser: (user) => {
            ...
            state.total += parseFloat(user.inputs.find((input) => input.token === token).format());
        },
    };
});
class StatefullPlanner {
    planner;
    universe;
    phases;
    userInput;
    balanceValues = new DefaultMap_1.DefaultMap(() => encodeArg(0, abi_1.ParamType.from('uint256')));
    predictedTokenBalances = new TokenAmounts_1.TokenAmounts();
    tokensHeldByExecutor = new Set();
    constructor(planner, universe, phases, userInput) {
        this.planner = planner;
    ...
    static create(planner, universe, phases, userInput) {
        return new StatefullPlanner(planner, universe, phases, userInput);
    }
    addInputQuantity(value) {
        this.predictedTokenBalances.add(value);
        this.balanceValues.set(value.token, encodeArg(this.predictedTokenBalances.get(value.token).amount, abi_1.ParamType.from('uint256')));
    }
    generatePlan(userInput) {
        this.predictedTokenBalances.add(userInput);
        this.tokensHeldByExecutor.add(userInput.token);
        let initialBalances = new TokenAmounts_1.TokenAmounts();
    ...
    tokenUsers = new DefaultMap_1.DefaultMap(() => new Set());
    analyzePath(path) {
        const needs = new TokenAmounts_1.TokenAmounts();
        const produces = new TokenAmounts_1.TokenAmounts();
    ...
    analyzePhase(phase, initialBalances) {
        const balancesPostPhase = initialBalances?.clone() ?? new TokenAmounts_1.TokenAmounts();
        const needs = new TokenAmounts_1.TokenAmounts();
        const produces = new TokenAmounts_1.TokenAmounts();
        phase.swapPaths.map((path) => {
            const { needs: pathNeeds, produces: pathProduces } = this.analyzePath(path);
...
exports.StatefullPlanner = StatefullPlanner;
...
const formatAddress = (address, universe) => {
    const addr = address.length === 66
        ? Address_1.Address.from('0x' + address.slice(26, 66))
        : Address_1.Address.from(address);
    ...
    const addrObj = Address_1.Address.from(address.length === 66 ? '0x' + address.slice(26, 66) : address);
    ...
    if (addr === Address_1.Address.ZERO) {
        return `address(0)`;
    }
...
const formatValue = (value, universe) => {
    if (value instanceof ReturnValue) {
        ...
        const out = abi_1.defaultAbiCoder.decode([value.param], value.value);
