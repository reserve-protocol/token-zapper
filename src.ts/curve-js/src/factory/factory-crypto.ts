...
function getCryptoFactoryUnderlyingCoinAddresses(
  ...
  return coinAddresses.map((coins: string[]) =>
    coins.map((c) =>
...
async function getCoinsData(
  ...
  const decimals = (res2.filter((a, i) => i % 2 == 1) as ethersBigNumber[]).map(
  ...
  for (let i = 0; i < poolIds.length; i++) {
    ...
    if (basePoolId) {
      ...
      CRYPTO_FACTORY_POOLS_DATA[poolIds[i]] = {
        ...
        is_factory: true,
        underlying_coins: underlyingCoinAddresses[i].map(
          (addr) => coinAddressNameDict[addr]
        ),
        wrapped_coins: coinAddresses[i].map(
          (addr) => coinAddressNameDict[addr]
        ...
        wrapped_coin_addresses: coinAddresses[i],
        underlying_decimals: underlyingCoinAddresses[i].map(
          (addr) => coinAddressDecimalsDict[addr]
        ),
        wrapped_decimals: coinAddresses[i].map(
          (addr) => coinAddressDecimalsDict[addr]
