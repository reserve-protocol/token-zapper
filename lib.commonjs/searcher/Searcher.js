...
class Searcher {
    ...
    async findTokenZapViaTrade(userInput, rToken, signerAddress, slippage, onResult, abortSignal, startTime = Date.now()) {
        ...
        let results = 0;
        let tolerance = 0.98;
        const invalue = parseFloat((await this.universe.fairPrice(inputTokenQuantity))?.toString() ?? '0');
        if (invalue > 50000) {
            tolerance = 0.997;
        }
    ...
    async zapIntoRToken(userInput, rToken, userAddress, opts) {
        ...
        const tradeZap = doTrades ? this.findTokenZapViaTrade(userInput, rToken, userAddress, slippage, controller.onResult, controller.abortController.signal, start).catch((e) => {
            errors.push(e);
        }) : Promise.resolve();
