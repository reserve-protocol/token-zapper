...
import { PricedTokenQuantity, Token, } from './entities/Token';
...
export class Universe {
    ...
    constructor(provider, config, approvalsStore, loadToken) {
        ...
        this.weirollZapperExec = Contract.createLibrary(ZapperExecutor__factory.connect(this.execAddress.address, this.provider));
        this.fairPriceCache = this.createCache(async (qty) => {
            return (this.oracle?.quote(qty).catch(() => this.usd.zero) ?? this.usd.zero);
    ...
    async zap(tokenIn, amountIn, rToken, signerAddress) {
        ...
        return this.searcher.findSingleInputToRTokenZap(inputTokenQty, outputToken, Address.from(signerAddress), this.config.defaultInternalTradeSlippage);
    }
    ...
    async redeem(rToken, amount, output, signerAddress) {
        ...
        return this.searcher.findRTokenIntoSingleTokenZap(inputTokenQty, outputToken, Address.from(signerAddress), this.config.defaultInternalTradeSlippage);
    }
}
