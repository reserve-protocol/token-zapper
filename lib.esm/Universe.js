...
export class Universe {
    ...
    createCache(fetch, ttl = this.config.requoteTolerance, keyFn) {
        const cache = new BlockCache(fetch, ttl, this.currentBlock, keyFn);
    ...
    preferredRTokenInputToken = new DefaultMap(() => new Set());
    preferredToken = new Map();
    addPreferredRTokenInputToken(token, inputToken) {
    ...
    constructor(provider, config, approvalsStore, loadToken, simulateZapFn_) {
        ...
        this.weirollZapperExecContract = Contract.createContract(ZapperExecutor__factory.connect(this.config.addresses.executorAddress.address, this.provider));
