...
class Searcher {
    ...
    async findRTokenIntoSingleTokenZapViaIssueance(rTokenQuantity, output, signerAddress, slippage = 0.0) {
        ...
        return new SearcherResult_1.BurnRTokenSearcherResult(this.universe, rTokenQuantity, outputSwap, signerAddress, outputToken);
    }
    async findTokenZapViaIssueance(userInput, rToken, signerAddress, slippage = 0.0) {
        await this.universe.initialized;
        return (0, controlflow_1.retryLoop)(() => this.findSingleInputToRTokenZap_(userInput, rToken, signerAddress, slippage).then((i) => [i]), {
            maxRetries: 2,
            retryDelay: 50,
        });
    }
    ...
    async findSingleInputToRTokenZap_(userInput, rToken, signerAddress, slippage = 0.0) {
        ...
        const searcherResult = new SearcherResult_1.MintRTokenSearcherResult(this.universe, userInput, new Swap_1.SwapPaths(this.universe, [inputTokenQuantity], [...inputQuantityToBasketTokens.swapPaths, rTokenMint], output, rTokenMint.outputValue, signerAddress), signerAddress, rToken);
