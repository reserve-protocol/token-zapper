...
class CurveSwap extends (0, Action_1.Action)('Curve') {
    ...
    async plan(planner, inputs, _, predicted) {
        ...
        const out = planner.add(curveRouterCallLib.exchange(inputs[0] ?? predicted[0].amount, minOut, routerContract.address, payload), `Curve,swap=${predicted.join(', ')} -> ${quoteWithout},pools=${[
            ...this.addressesInUse,
        ].join(', ')}`, `amt_${this.outputToken[0].symbol}`);
        if (out == null) {
    ...
    async _quote(amountsIn) {
        ...
        try {
            ...
            for (const r of _route) {
                this.addPool(Address_1.Address.from(r));
            }
            for (const a of _factorySwapAddresses) {
                this.addPool(Address_1.Address.from(a));
            }
            const out_ = await contract.callStatic.get_exchange_multiple_amount(_route, _swapParams, amountsIn.amount, _factorySwapAddresses, {
                blockTag: 'pending',
            });
