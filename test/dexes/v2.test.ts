...
describe("dexes/v2", () => {
    it("standard impl", async () => {
        ...
        const pool = V2Pool.createStandardV2Pool(
            UniV2Factory,
            USDT,
            WETH,
            3000n
        ...
        const swap01 = await pool.swapFn(
            WETH.fromDecimal("1"),
            {
                pool,
                inputToken: WETH,
        ...
        const swap10 = await pool.swapFn(
            USDT.fromDecimal("1000"),
            {
                pool,
                inputToken: USDT,
                outputToken: WETH,
