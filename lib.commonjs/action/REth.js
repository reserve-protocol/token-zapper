...
exports.RETHToETH = exports.RETHToWETH = exports.WETHToRETH = exports.ETHToRETH = exports.REthRouter = void 0;
...
class WETHToRETH extends RocketPoolBase {
    ...
    async plan(planner, [input], _, [inputPrecomputed]) {
        ...
        if (f0 !== 0n && f1 !== 0n) {
            // Using a helper library we
            const input0 = planner.add(zapperLib.fpMul(f0, inp, ONE), `input * ${(0, utils_1.formatEther)(f0)}`, 'frac0');
            const input1 = planner.add(zapperLib.fpMul(f1, inp, ONE), `input * ${(0, utils_1.formatEther)(f1)}`, 'frac1');
    ...
    constructor(universe, router) {
        super(universe.wrappedNativeToken.address, [universe.wrappedNativeToken], [router.reth], Action_1.InteractionConvention.None, Action_1.DestinationOptions.Callee, []);
        this.universe = universe;
        this.router = router;
    }
    get supportsDynamicInput() {
...
class RETHToWETH extends RocketPoolBase {
    ...
    constructor(universe, router) {
        super(universe.wrappedNativeToken.address, [router.reth], [universe.wrappedNativeToken], Action_1.InteractionConvention.ApprovalRequired, Action_1.DestinationOptions.Callee, [new Approval_1.Approval(router.reth, Address_1.Address.from(router.routerInstance.address))]);
        this.universe = universe;
        this.router = router;
    }
}
exports.RETHToWETH = RETHToWETH;
class RETHToETH extends RocketPoolBase {
    universe;
    router;
    get action() {
        return 'swapFrom';
    ...
    constructor(universe, router) {
        super(universe.nativeToken.address, [router.reth], [universe.nativeToken], Action_1.InteractionConvention.ApprovalRequired, Action_1.DestinationOptions.Callee, [new Approval_1.Approval(router.reth, Address_1.Address.from(router.routerInstance.address))]);
        this.universe = universe;
        this.router = router;
    }
}
