    ...
    async toTransaction(options = {}) {
        ...
        try {
            ...
            for (const mintPath of this.parts.minting.swapPaths) {
                for (const step of mintPath.steps) {
                    if (step.action instanceof RTokens_1.MintRTokenAction) {
                        await step.action.plan(this.planner, step.inputs.map((i) => (0, Planner_1.encodeArg)(i.amount, utils_1.ParamType.from('uint256'))), this.universe.config.addresses.executorAddress);
