...
class CurveSwap extends Action_1.Action {
    ...
    async plan(planner, inputs, _, [amountsIn]) {
        ...
        const out = planner.add(curveRouterCallLib.exchange(inputs[0] ?? amountsIn.amount, minOut.amount, routerContract.address, payload), `Curve,swap=${amountsIn} -> ${minOut}`, `amt_${this.output[0].symbol}`);
        return [out];
    }
    estimate;
    gasEstimate() {
