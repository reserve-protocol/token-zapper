...
export const generateAllPermutations = async function (searcher, arr, precursorTokens, signer) {
    ...
    const withoutComflicts = allCombos.filter((paths) => willPathsHaveAddressConflicts(searcher.debugLog, universe, paths, signer)
        .length === 0);
