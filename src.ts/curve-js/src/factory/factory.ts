...
async function getRecentlyCreatedPoolId(this: any, swapAddress: string): Promise<string> {
...
async function getFactoryIdsAndSwapAddresses(this: any, fromIdx = 0): Promise<[string[], string[]]> {
    const factoryContract = this.contracts[this.constants.ALIASES.factory].contract;
...
function _handleReferenceAssets(referenceAssets: ethersBigNumber[]): REFERENCE_ASSET[] {
...
async function getPoolsData(this: ICurve, factorySwapAddresses: string[]): Promise<[string[], string[], REFERENCE_ASSET[], string[], string[], boolean[], string[][]]> {
    ...
    const referenceAssets = _handleReferenceAssets(res.filter((a, i) => i % 7 == 2) as ethersBigNumber[]);
...
async function getCoinsData(
    ...
    const decimals = (res.filter((a, i) => i % 2 == 1) as ethersBigNumber[]).map((_d) => Number(formatUnits(_d, 0)));
