...
class SearcherResult {
    ...
    async simulateNoNode({ data, value }) {
        ...
        if (resp.startsWith('0x08c379a0')) {
            const msg = abi_1.defaultAbiCoder.decode(['string'], '0x' + resp.slice(10))[0];
            throw new Error(msg);
        }
    ...
    async toTransaction(options = {}) {
        ...
        const value = inputIsNativeToken ? bignumber_1.BigNumber.from(payload.amountIn) : constants_1.Zero;
        ...
        const totalValue = (await this.universe.fairPrice(outputTokenOutput))?.add(valueOfDust) ??
            valueOfDust;
        this.swaps = new Swap_1.SwapPaths(this.universe, this.swaps.inputs, this.swaps.swapPaths, simulatedOutputs, totalValue, this.swaps.destination);
        if (outputIsRToken && options.maxIssueance !== true) {
            ...
            const mintRtoken = this.universe.wrappedTokens.get(this.outputToken)
                .mint;
            this.swaps.swapPaths.push(new Swap_1.SwapPath(previous.inputs, previous.steps, simulatedOutputs, totalValue, previous.destination));
            builder.addCall(await mintRtoken.encodeIssueTo(previous.inputs, 
            // TODO: Find a better way to avoid off by one errors
            outputTokenOutput.token.from(outputTokenOutput.amount - outputTokenOutput.amount / 3000000n), this.signer));
        }
        if (options.returnDust) {
            builder.drainERC20(dustQuantities.map((i) => i.token), this.signer);
        }
        this.swaps = new Swap_1.SwapPaths(this.universe, this.swaps.inputs, this.swaps.swapPaths, simulatedOutputs.map((i) => {
            if (i.token === this.outputToken) {
                return i.token.from(outputTokenOutput.amount - outputTokenOutput.amount / 250000n);
            }
            return i;
        ...
        payload = {
            ...
            amountIn: this.swaps.inputs[0].amount,
            commands: builder.contractCalls.map((i) => i.encode()),
            amountOut: outputTokenOutput.amount - outputTokenOutput.amount / 250000n,
            tokenOut: amountOut.token.address.address,
            tokensUsedByZap: dustTokens.map((i) => i.address.address),
        };
        ...
        if (this.universe.chainId === 1) {
            ...
            tx = {
                ...tx,
                type: 0,
                gasPrice: bignumber_1.BigNumber.from(this.universe.gasPrice),
            };
        }
        const out = new ZapTransaction_1.ZapTransaction(payload, tx, zapperResult2.gasUsed.toBigInt(), this.swaps.inputs[0], this.swaps.outputs);
        return out;
    }
}
