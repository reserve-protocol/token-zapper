...
export const resolveTradeConflicts = async (
  ...
  try {
    ...
    for (const [_, trades] of sameFirstPool.entries()) {
      ...
      const {
        inputToken: [inputToken], currentQuote: {
...
export class MultiChoicePath implements SwapPath {
  ...
  mustHasBalance(balances: TokenAmounts) {
    if (this.paths.every((i) => balances.hasBalance(i.inputs))) {
      return this;
    }
    return new MultiChoicePath(
      this.universe,
      this.paths.filter((p) => balances.hasBalance(p.inputs))
    );
  }
