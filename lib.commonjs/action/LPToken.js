...
class LPTokenMint extends (0, Action_1.Action)('LPPlaceholder') {
    lpToken;
    async plan(planner, inputs, destination) {
        throw new Error(`Method not implemented. For ${this.inputToken.join(', ')} -> ${this.outputToken.join(', ')}`);
    }
    toString() {
...
class LPTokenBurn extends (0, Action_1.Action)('LPPlaceholder') {
    lpToken;
    async plan(planner, inputs, destination) {
