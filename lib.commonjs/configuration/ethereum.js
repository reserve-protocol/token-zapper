...
const initialize = async (universe) => {
    ...
    await universe.defineRToken(universe.config.addresses.rTokenDeployments['ETH+']);
    await universe.defineRToken(universe.config.addresses.rTokenDeployments.hyUSD);
    ...
    if (universe.chainConfig.config.curveConfig.enable) {
        ...
        const eUSDConvex = await (0, Convex_1.setupConvexEdges)(universe, stkcvxeUSD3CRV);
        const stkcvxMIM3LP3CRV = await universe.getToken(Address_1.Address.from('0x8443364625e09a33d793acd03aCC1F3b5DbFA6F6'));
        const mimConvex = await (0, Convex_1.setupConvexEdges)(universe, stkcvxMIM3LP3CRV);
    ...
    // Compound
    await (0, loadCompound_1.setupCompoundLike)(universe, wrappedToUnderlyingMapping, {
