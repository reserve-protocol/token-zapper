...
export const setupEthereumZapper = async (universe) => {
    ...
    const depositToETHX = new (ETHTokenVaultDepositAction('ETHX'))(universe, universe.commonTokens.ETHx, Address.from('0xcf5EA1b38380f6aF39068375516Daf40Ed70D299'), 1n);
    ...
    universe.tokenTradeSpecialCases.set(universe.commonTokens.ETHx, async (input, dest) => {
        if (input.token === universe.wrappedNativeToken) {
            return await new SwapPlan(universe, [depositToETHX]).quote([input], dest);
        }
        return null;
    });
