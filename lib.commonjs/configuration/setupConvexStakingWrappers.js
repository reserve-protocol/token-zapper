    ...
    async attachToUniverse() {
        ...
        for (const token of curvePool.allPoolTokens) {
            ...
            handlers.set(token, async (unit) => {
                ...
                return BasketTokenSourcingRules_1.BasketTokenSourcingRuleApplication.singleBranch([inputQty], [
                    BasketTokenSourcingRules_1.PostTradeAction.fromAction(edge, true),
                    BasketTokenSourcingRules_1.PostTradeAction.fromAction(this.curveLpToWrapper, true),
                ]);
            });
        }
        const oracle = PriceOracle_1.PriceOracle.createSingleTokenOracle(this.universe, this.wrapperToken, async () => (await this.universe.fairPrice(this.curvePool.lpToken.one)) ??
        ...
        for (const baseTok of this.curvePool.allPoolTokens) {
            try {
                ...
                if ((0, Action_1.isMultiChoiceEdge)(act)) {
