        ...
        await Promise.all(inputPrTrade.map(async ({ input, output }) => {
            ...
            for (let i = 1; i <= 3; i++) {
                try {
                    ...
                    const potentialSwaps = await this.findSingleInputTokenSwap(mustBeDynamic, input, output, this.universe.config.addresses.executorAddress, internalTradeSlippage, abortSignal, i);
