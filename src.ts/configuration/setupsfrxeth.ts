...
export const setupFrxETH = async (
  ...
  const frxETHToETH = await universe.integrations.curve!.createTradeEdge(
    frxETH,
    universe.nativeToken
  )!

  const frxETHToWETH: BaseAction = new (frxETHToETH!.combine(
    universe.graph.graph
      .get(universe.nativeToken)
      .get(universe.wrappedNativeToken)![0]
  ))(universe)
  universe.addAction(frxETHToWETH)
}
