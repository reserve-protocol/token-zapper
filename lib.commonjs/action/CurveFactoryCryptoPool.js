...
class CurveFactoryCryptoPoolAddLiquidityAction extends (0, Action_1.Action)('CurveFactoryCryptoPool') {
    ...
    async plan(planner, [input], destination, [amountIn]) {
        ...
        const tradeInputSplit = this.genUtils.fraction(this.universe, planner, input, quote.tradeFraction, ` of ${this.inputToken} into ${quote.subTrade.action.toString()}`, `input_trade`);
        ...
        const actionInput = this.genUtils.fraction(this.universe, planner, input, Action_1.ONE - quote.tradeFraction, ` of ${this.inputToken} into ${this}`, `input_lpdeposit`);
    ...
    async quoteInner(amountIn) {
        ...
        if (this.tokenIndex === 0) {
            ...
            tradeFraction = Action_1.ONE - fractionToken1.amount;
            ...
            const amountQty = amountIn.mul(fractionToken0);
            const tradeQty = amountIn.sub(amountQty);
            tradeFraction = Action_1.ONE - fractionToken0.amount;
