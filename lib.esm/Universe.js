...
export class Universe {
    ...
    constructor(provider, config, approvalsStore, loadToken, simulateZapFn) {
        this.provider = provider;
        this.config = config;
        this.approvalsStore = approvalsStore;
        this.loadToken = loadToken;
        this.simulateZapFn = simulateZapFn;
        ...
        this.fairPriceCache = this.createCache(async (qty) => {
            if (this.rTokenDeployments.has(qty.token)) {
                const outs = await this.rTokenDeployments.get(qty.token).burn.quote([qty]);
