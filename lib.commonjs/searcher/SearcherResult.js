...
class BaseSearcherResult {
    ...
    async customSimulator(opts) {
        ...
        try {
            const [[dust, amountOut, gasUsed]] = utils_1.defaultAbiCoder.decode(['(uint256[],uint256,uint256)'], resultOfZap.success.value);
            const out = {
                0: dust,
                dust,
                1: amountOut,
    ...
    async createZapTransaction(options) {
        try {
            ...
            const stats = await this.universe.perf.measurePromise('ZapTxStats.create', ZapTransaction_1.ZapTxStats.create(this.universe, {
