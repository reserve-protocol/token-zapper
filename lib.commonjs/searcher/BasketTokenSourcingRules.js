...
class BasketTokenSourcingRuleApplication {
    ...
    static fromBranches(branches, action) {
        const precursors = Token_1.TokenAmounts.fromQuantities(branches.map((i) => i.precursorToTradeFor).flat());
        const subActions = branches.map((i) => new PostTradeAction(Token_1.TokenAmounts.fromQuantities(i.precursorToTradeFor)
    ...
    static fromActionWithDependencies(action, branches) {
        const precursors = Token_1.TokenAmounts.fromQuantities(branches.map((i) => i.precursorToTradeFor).flat());
        const execFirst = branches.map((i, ith) => new PostTradeAction(Token_1.TokenAmounts.fromQuantities(i.precursorToTradeFor)
            .recalculateAsFractionOf(precursors)
