...
export class BurnRTokenSearcherResult extends BaseSearcherResult {
    ...
    async toTransaction(options = {}) {
        ...
        const outputs = await this.parts.rtokenRedemption.steps[0].action
            .plan(this.planner, [encodeArg(this.userInput.amount, ParamType.fromString('uint256'))], this.universe.config.addresses.executorAddress, [this.userInput])
            .catch((a) => {
        ...
        try {
            ...
            for (const trade of this.parts.trading.swapPaths) {
                for (const step of trade.steps) {
                    ...
                    if (recalcInputTokenPostTrade) {
                        trades.set(inputToken, plannerUtils.erc20.balanceOf(this.universe, this.planner, inputToken, this.universe.config.addresses.executorAddress));
                    }
            ...
            for (const tok of this.parts.minting.inputs) {
                if (!trades.has(tok.token)) {
                    trades.set(tok.token, plannerUtils.erc20.balanceOf(this.universe, this.planner, tok.token, this.universe.config.addresses.executorAddress));
                }
            }
            for (const mintPath of this.parts.minting.swapPaths) {
                for (const step of mintPath.steps) {
