...
const setupUniswapRouter = async (universe) => {
    ...
    const ethEstimateGasSimulator = new smart_order_router_1.EthEstimateGasSimulator(universe.chainId, universe.provider, v2PoolProvider, v3PoolProvider, portionProvider);
    ...
    const computeRoute = async (abort, input, output, slippage) => {
        ...
        const route = await legacy.route(inp, outp, sdk_core_1.TradeType.EXACT_INPUT, {
