...
export class Searcher {
     ...
     **/
    async *findSingleInputToBasketGivenBasketUnit(inputQuantity, rToken, basketUnit, internalTradeSlippage) {
        /**
        ...
        await Promise.all(inputPrTrade.map(async ({ input, output }) => {
            ...
            const trade = await this.findSingleInputTokenSwap(input, output, this.universe.config.addresses.executorAddress, internalTradeSlippage, 2, false);
            if (trade == null) {
    ...
    async findRTokenIntoSingleTokenZapTx(rTokenQuantity, output, signerAddress, opts) {
        ...
        const [mintResults, tradeResults] = await Promise.all([
            this.findRTokenIntoSingleTokenZapViaRedeem(rTokenQuantity, output, signerAddress, toTxArgs.internalTradeSlippage),
            this.findTokenZapViaTrade(rTokenQuantity, output, signerAddress, toTxArgs.internalTradeSlippage),
        ]);
    ...
    async findBestTx(allQuotes, toTxArgs) {
        ...
        let quotes = (await Promise.all(allQuotes.map(async (q) => {
            try {
        ...
        const txes = await Promise.all(quotes.map(async (a) => {
            try {
                return {
    ...
    async findRTokenIntoSingleTokenZap(rTokenQuantity, output, signerAddress, internalTradeSlippage) {
        ...
        const [mintResults, tradeResults] = await Promise.all([
            this.findRTokenIntoSingleTokenZapViaRedeem(rTokenQuantity, output, signerAddress, internalTradeSlippage),
            this.findTokenZapViaTrade(rTokenQuantity, output, signerAddress, internalTradeSlippage),
        ]);
    ...
    async findRTokenIntoSingleTokenZapViaRedeem(rTokenQuantity, output, signerAddress, slippage) {
        const out = [];
    ...
    async *findRTokenIntoSingleTokenZapViaRedeem__(rTokenQuantity, output, signerAddress, slippage) {
        await this.universe.initialized;
        ...
        const trades = await Promise.all(unwrapTokenQtys
            ...
            .map(async (qty) => {
            const potentialSwaps = await this.findSingleInputTokenSwap(qty, outputToken, signerAddress, slippage, 2, true);
    ...
    async findTokenZapViaIssueance(userInput, rToken, signerAddress, slippage) {
        await this.universe.initialized;
        const outputs = [];
    ...
    async findTokenZapViaTrade(userInput, rToken, signerAddress, slippage) {
        await this.universe.initialized;
        ...
        const [paths, inputValue] = await Promise.all([
            this.externalQuoters(inputTokenQuantity, rToken, this.universe.execAddress, false, slippage),
    ...
    async findSingleInputToRTokenZapTx(userInput, rToken, userAddress, opts) {
        ...
        this.checkIfSimulationSupported();
        const [mintResults, tradeResults] = await Promise.all([
            this.findTokenZapViaIssueance(userInput, rToken, userAddress, slippage).catch(() => []),
            this.findTokenZapViaTrade(userInput, rToken, userAddress, slippage),
        ]);
        return await this.findBestTx([...mintResults, ...tradeResults], toTxArgs);
    }
    async *findSingleInputToRTokenZap_(userInput, rToken, signerAddress, slippage) {
        const inputIsNative = userInput.token === this.universe.nativeToken;
        let inputTokenQuantity = userInput;
        if (inputIsNative) {
            if (this.universe.commonTokens.ERC20GAS == null) {
        ...
        for await (const inputQuantityToBasketTokens of this.findSingleInputToBasketGivenBasketUnit(inputTokenQuantity, rToken, unitBasket, slippage)) {
            try {
                ...
                yield new MintRTokenSearcherResult(this.universe, userInput, parts, signerAddress, rToken);
            }
            catch (e) {
                // console.log(e)
            }
    ...
    async externalQuoters(input, output, destination, dynamicInput, slippage, onResult) {
        const out = [];
        await this.externalQuoters_(AbortSignal.timeout(this.config.routerDeadline), input, output, destination, dynamicInput, slippage, (path) => {
            out.push(path);
        }).catch((e) => {
            console.log(e);
        });
    ...
    async externalQuoters_(abort, input, output, destination, dynamicInput, slippage, onResult) {
        ...
        await Promise.all(aggregators.map(async (router) => {
            try {
                onResult(await router.swap(abort, executorAddress, destination, input, output, slippage));
            }
            catch (e) { }
        }));
    }
    async internalQuoter(input, output, destination, maxHops = 2) {
    ...
    async findSingleInputTokenSwap(input, output, destination, slippage, maxHops = 2, dynamicInput = false) {
