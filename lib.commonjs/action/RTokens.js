...
class BurnRTokenAction extends Action_1.Action {
    ...
    async encode([quantity], destination) {
        return new ContractCall_1.ContractCall((0, utils_1.parseHexStringIntoBuffer)(rTokenIFace.encodeFunctionData('redeem', [quantity.amount])), this.basketHandler.rToken.address, 0n, this.gasEstimate(), `Burn RToken(${this.output.join(",")},input:${quantity},destination: ${destination})`);
    }
