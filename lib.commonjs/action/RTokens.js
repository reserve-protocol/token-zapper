...
class MintRTokenAction extends (0, Action_1.Action)("Reserve") {
    universe;
    basket;
    async plan(planner, inputs, destination) {
        const lib = this.gen.Contract.createContract(contracts_1.IRToken__factory.connect(this.inputToken[0].address.address, this.universe.provider));
...
class BurnRTokenAction extends (0, Action_1.Action)("Reserve") {
    universe;
    basketHandler;
    get outputSlippage() {
        return 300000n;
    ...
    async plan(planner, inputs, destination, predicted) {
        ...
        return this.outputToken.map((token) => this.genUtils.erc20.balanceOf(this.universe, planner, token, this.universe.config.addresses.executorAddress));
    }
    gasEstimate() {
        return BigInt(600000n);
    }
