...
export const setupEthereumZapper = async (universe) => {
    ...
    const wrappedToUnderlyingMapping = (await import('./data/ethereum/underlying.json', { assert: { type: 'json' } })).default;
    // Set up compound
    ...
    // Set up flux finance
    const fTokens = await convertWrapperTokenAddressesIntoWrapperTokenPairs(universe, PROTOCOL_CONFIGS.fluxFinance.markets, wrappedToUnderlyingMapping);
    await setupCompoundLike(universe, {
