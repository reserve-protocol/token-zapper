...
class Searcher {
    ...
    async findSingleInputToBasketGivenBasketUnit(inputQuantity, rToken, basketUnit, slippage) {
        ...
        await Promise.all(inputPrTrade.map(async ({ input, output }) => {
            ...
            const swaps = await this.findSingleInputTokenSwap(input, output, this.universe.config.addresses.executorAddress, slippage, 1, false);
            const trade = swaps[0];
            if (trade == null) {
    ...
    async findTokenZapViaIssueance(userInput, rToken, signerAddress, slippage = 0.0) {
        await this.universe.initialized;
        const out = await this.findSingleInputToRTokenZap_(userInput, rToken, signerAddress, slippage);
        return [out];
    }
