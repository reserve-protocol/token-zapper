...
const ETHTokenVaultDepositAction = (proto) => class ETHTokenVaultDepositAction extends (0, Action_1.Action)(proto) {
    ...
    async plan(planner, inputs, destination, predicted) {
        ...
        const weth = this.gen.Contract.createContract(contracts_1.IWrappedNative__factory.connect(this.universe.wrappedNativeToken.address.address, this.universe.provider));
    ...
    constructor(universe, shareToken, vaultAddress, slippage) {
        super(shareToken.address, [universe.wrappedNativeToken], [shareToken], Action_1.InteractionConvention.ApprovalRequired, Action_1.DestinationOptions.Recipient, [new Approval_1.Approval(universe.wrappedNativeToken, shareToken.address)]);
        this.universe = universe;
        this.shareToken = shareToken;
        this.vaultAddress = vaultAddress;
        this.slippage = slippage;
        this.inst = contracts_1.ETHTokenVault__factory.connect(vaultAddress.address, this.universe.provider);
