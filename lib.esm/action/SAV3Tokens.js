...
export class MintSAV3TokensAction extends Action {
    ...
    async plan(planner, inputs, destination, predicted) {
        const lib = this.gen.Contract.createContract(IStaticAV3TokenLM__factory.connect(this.outputToken[0].address.address, this.universe.provider));
...
export class BurnSAV3TokensAction extends Action {
    ...
    async plan(planner, inputs, destination, predicted) {
        const lib = this.gen.Contract.createContract(IStaticAV3TokenLM__factory.connect(this.inputToken[0].address.address, this.universe.provider));
    ...
    async quote([amountsIn]) {
        IStaticAV3TokenLM__factory.connect(this.inputToken[0].address.address, this.universe.provider);
        await this.universe.refresh(this.address);
        return [
            this.saToken
    ...
    constructor(universe, underlying, saToken, rate) {
        ...
        this.saToken = saToken;
        this.rate = rate;
        this.inst = IStaticAV3TokenLM__factory.connect(saToken.address.address, universe.provider);
    }
    toString() {
