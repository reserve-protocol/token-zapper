...
export class MintSAV3TokensAction extends Action('AaveV3') {
    ...
    async plan(planner, inputs, destination, predicted) {
        ...
        planner.add(lib.deposit(inp, this.universe.execAddress.address, 0, true), `IStaticATokenV3LM mint: ${predicted.join(', ')} -> ${await this.quote(predicted)}`);
...
export class BurnSAV3TokensAction extends Action('AaveV3') {
    ...
    async plan(planner, inputs, destination, predicted) {
        ...
        planner.add(lib.redeem(inputs[0], destination.address, this.universe.execAddress.address, true), `IStaticATokenV3LM burn: ${predicted.join(', ')} -> ${await this.quote(predicted)}`);
        return this.outputBalanceOf(this.universe, planner);
    }
    gasEstimate() {
        return BigInt(300000n);
