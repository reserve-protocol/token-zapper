...
export class Searcher {
    ...
    async findSingleInputToBasketGivenBasketUnit(inputQuantity, rToken, basketUnit, internalTradeSlippage, onResult, abortSignal) {
        ...
        if (initialTrade != null) {
            ...
            firstTrade = (await this.findSingleInputTokenSwap(false, initialTrade.input, initialTrade.output, this.universe.execAddress, this.defaultInternalTradeSlippage, AbortSignal.timeout(this.config.routerDeadline), 1)).path;
        ...
        await Promise.all(inputPrTrade.map(async ({ input, output }) => {
            ...
            for (let i = 1; i <= 3; i++) {
                try {
                    ...
                    const potentialSwaps = await this.findSingleInputTokenSwap(firstTrade != null, input, output, this.universe.config.addresses.executorAddress, internalTradeSlippage, abortSignal, i);
        ...
        if (precursorTokens.precursorToTradeFor.length == 1 &&
            ...
            return await onResult(await generateIssueancePlan(await resolveTradeConflicts(this, abortSignal, [])));
        }
        ...
        const allOptions = await this.perf.measurePromise('generateAllPermutations', generateAllPermutations(this, multiTrades, precursorSet), rToken.symbol);
        const aborter = new AbortController();
        const candidateChunks = chunkifyIterable(allOptions, this.maxConcurrency, abortSignal);
    ...
    async findRTokenIntoSingleTokenZapViaRedeem__(rTokenQuantity, output, signerAddress, slippage, onResult, abortSignal, startTime) {
        ...
        if (permutableTrades.length === 0) {
            ...
            const allposibilities = await generateAllPermutations(this, permutableTrades, new Set([outputToken]));
    ...
    async findSingleInputToRTokenZap_(userInput, rToken, signerAddress, slippage, onResult, abort, endPosition = rToken, startTime = Date.now()) {
        ...
        await this.findSingleInputToBasketGivenBasketUnit(inputTokenQuantity, rToken, unitBasket, slippage, async (inputQuantityToBasketTokens) => {
            ...
            if (endPosition !== rToken) {
                const lastSteps = await this.findSingleInputTokenSwap(true, tradingBalances.get(rToken), endPosition, signerAddress, slippage, abort, 2);
