...
class Searcher {
        ...
        await Promise.all(inputPrTrade.map(async ({ input, output }) => {
            ...
            for (let i = 1; i < 3; i++) {
                try {
                    ...
                    const potentialSwaps = await this.findSingleInputTokenSwap(input, output, this.universe.config.addresses.executorAddress, internalTradeSlippage, abortSignal, Math.max(i, 2), false);
    ...
    async findRTokenIntoSingleTokenZapTx(rTokenQuantity, output, signerAddress, opts) {
        ...
        void Promise.race([
            this.findRTokenIntoSingleTokenZapViaRedeem__(rTokenQuantity, output, signerAddress, toTxArgs.internalTradeSlippage, controller.onResult, controller.abortController.signal, start).catch(() => { }),
            this.findTokenZapViaTrade(rTokenQuantity, output, signerAddress, toTxArgs.internalTradeSlippage, controller.abortController.signal, start)
                .catch((e) => [])
    ...
    async findRTokenIntoSingleTokenZap(rTokenQuantity, output, signerAddress, internalTradeSlippage) {
        ...
        const [mintResults, tradeResults] = (await Promise.all([
            this.findRTokenIntoSingleTokenZapViaRedeem(rTokenQuantity, output, signerAddress, internalTradeSlippage, timeout, start),
            this.findTokenZapViaTrade(rTokenQuantity, output, signerAddress, internalTradeSlippage, timeout, start),
        ]));
    ...
    async findRTokenIntoSingleTokenZapViaRedeem(rTokenQuantity, output, signerAddress, slippage, abortSignal, startTime = Date.now()) {
        const out = [];
        await this.findRTokenIntoSingleTokenZapViaRedeem__(rTokenQuantity, output, signerAddress, slippage, async (burnZap) => {
            out.push(burnZap);
        }, abortSignal, startTime);
        return out;
    }
    async findRTokenIntoSingleTokenZapViaRedeem__(rTokenQuantity, output, signerAddress, slippage, onResult, abortSignal, startTime) {
        ...
        const generateRedeemPlan = async (trades = []) => {
            ...
            return new SearcherResult_1.RedeemZap(this.universe, rTokenQuantity, {
                full: outputSwap,
                rtokenRedemption: redeem,
                tokenBasketUnwrap: redeemSwapPaths,
                tradesToOutput: trades,
            }, signerAddress, outputToken, startTime, abortSignal);
        };
    ...
    async findTokenZapViaTrade(userInput, rToken, signerAddress, slippage, abortSignal, startTime = Date.now()) {
        await this.universe.initialized;
        ...
        return paths
            ...
            .slice(0, 2)
            .map((path) => new SearcherResult_1.ZapViaATrade(this.universe, userInput, new Swap_1.SwapPaths(this.universe, [inputTokenQuantity], [path], path.outputs, path.outputValue, signerAddress), signerAddress, rToken, startTime, abortSignal));
    }
    ...
    async findSingleInputToRTokenZapTx(userInput, rToken, userAddress, opts) {
        ...
        const controller = createConcurrentStreamingSeacher(this, toTxArgs);
        void Promise.race([
            this.findSingleInputToRTokenZap_(userInput, rToken, userAddress, slippage, controller.onResult, controller.abortController.signal, start).catch((e) => { }),
            this.findTokenZapViaTrade(userInput, rToken, userAddress, slippage, controller.abortController.signal, start).then(async (i) => {
                try {
        ...
        try {
            await this.findSingleInputToBasketGivenBasketUnit(inputTokenQuantity, rToken, unitBasket, slippage, async (inputQuantityToBasketTokens) => {
                try {
                    ...
                    await onResult(new SearcherResult_1.MintZap(this.universe, userInput, parts, signerAddress, rToken, startTime, abort));
                }
                catch (e) {
                    // console.log(e)
                    // console.log(e.stack)
