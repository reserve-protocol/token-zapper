...
class SFrxETHMint extends BaseFrxETH {
    ...
    constructor(universe, frxeth, sfrxeth, vault) {
        super(sfrxeth.address, [frxeth], [sfrxeth], Action_1.InteractionConvention.ApprovalRequired, Action_1.DestinationOptions.Callee, []);
        this.universe = universe;
        this.frxeth = frxeth;
    ...
    async quote(amountsIn) {
        return [
            this.sfrxeth.from(await this.vault.previewDeposit(amountsIn[0].amount))
        ];
    }
}
class SFrxETHburn extends BaseFrxETH {
    ...
    async quote(amountsIn) {
        return [
            this.frxeth.from(await this.vault.previewRedeem(amountsIn[0].amount))
        ];
    }
}
const setupFrxETH = async (universe, config) => {
    ...
    const sfrxEthOracle = PriceOracle_1.PriceOracle.createSingleTokenOracle(universe, sfrxETH, () => burnSfrxETH
        .quote([sfrxETH.one])
        .then((o) => frxEthOracle
        .quote(o[0].token)
