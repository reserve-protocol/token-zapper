...
const Searcher_2 = require("./searcher/Searcher");
...
class Universe {
    ...
    async canZapIntoRToken(token) {
        ...
        if (!(wrappable.mint instanceof RTokens_1.MintRTokenAction)) {
            throw new Error('Not an rToken');
        }
        const action = wrappable.mint;
        const unit = action.basket.unitBasket;
        const searcher = new Searcher_1.Searcher(this);
        const input = this.nativeToken.from('0.1');
        const precursorSet = await (0, Searcher_2.findPrecursorTokenSet)(this, input, token, unit, searcher);
        let tokensMissings = [];
