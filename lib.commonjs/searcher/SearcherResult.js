...
class BaseSearcherResult {
    ...
    async simulateNoNode({ data, value }) {
        try {
            ...
            console.log('error:', e.message, 'Failing program:', (0, Planner_1.printPlan)(this.planner, this.universe)
                .map((i) => '  ' + i)
                .join('\n'));
        }
    ...
    async toTransactionWithRetry(opts) {
        ...
        for (let i = 0; i < 3; i++) {
            try {
                ...
                await (0, controlflow_1.wait)(300);
    ...
    async createZapTransaction(options) {
        try {
            ...
            console.log((0, Planner_1.printPlan)(this.planner, this.universe).join('\n'));
