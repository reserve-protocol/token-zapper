...
import { AlphaRouter, CachingGasStationProvider, CachingTokenListProvider, CachingTokenProviderWithFallback, CachingV2PoolProvider, CachingV3PoolProvider, CachingV4PoolProvider, CurrencyAmount, EIP1559GasPriceProvider, EthEstimateGasSimulator, LegacyGasPriceProvider, LegacyRouter, NodeJSCache, OnChainGasPriceProvider, OnChainQuoteProvider, SwapType, TokenPropertiesProvider, TokenProvider, UniswapMulticallProvider, V2PoolProvider, V3PoolProvider, V4PoolProvider, } from '@uniswap/smart-order-router';
...
export class UniswapRouterAction extends Action('Uniswap') {
    ...
    constructor(currentQuote, universe, dex) {
        super(currentQuote.to, [currentQuote.input.token], [currentQuote.outputWithSlippage.token], InteractionConvention.ApprovalRequired, DestinationOptions.Recipient, [new Approval(currentQuote.input.token, currentQuote.to)]);
...
export const setupUniswapRouter = async (universe) => {
    ...
    const v4PoolProvider = new CachingV4PoolProvider(universe.chainId, new V4PoolProvider(universe.chainId, multicall), new NodeJSCache(new NodeCache({ stdTTL: 360, useClones: false })));
    const portionProvider = new PortionProvider();
    const ethEstimateGasSimulator = new EthEstimateGasSimulator(universe.chainId, universe.provider, v2PoolProvider, v3PoolProvider, v4PoolProvider, portionProvider);
    const gasPriceCache = new NodeJSCache(new NodeCache({ stdTTL: 15, useClones: true }));
    const gasPriceProvider = new CachingGasStationProvider(universe.chainId, new OnChainGasPriceProvider(universe.chainId, new EIP1559GasPriceProvider(universe.provider), new LegacyGasPriceProvider(universe.provider)), gasPriceCache);
    ...
    const computeRoute = async (abort, input, output, slippage) => {
        ...
        const route = universe.chainId === 1
            ? await alphaRouter.route(inp, outp, TradeType.EXACT_INPUT, {
