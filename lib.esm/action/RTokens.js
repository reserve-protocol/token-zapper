...
export class MintRTokenAction extends ReserveRTokenBase {
    rTokenDeployment;
    action = 'issue';
    async plan(planner, _, destination, predictedInput) {
        ...
        if (totalSupply.isZero()) {
            ...
            planner.add(Contract.createContract(this.rTokenDeployment.contracts.rToken).issueTo(destination.address, quote.amount));
        }
        else {
    ...
    constructor(rTokenDeployment) {
        super(rTokenDeployment.rToken.address, rTokenDeployment.basket, [rTokenDeployment.rToken], InteractionConvention.ApprovalRequired, DestinationOptions.Recipient, rTokenDeployment.basket.map((input) => new Approval(input, rTokenDeployment.rToken.address)));
