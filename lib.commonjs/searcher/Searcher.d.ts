...
export declare class Searcher<SearcherUniverse extends Universe<Config>> {
    readonly universe: SearcherUniverse;
    private readonly defaultSearcherOpts;
    private internalQuoerCache;
    private externalQuoteCache;
     ...
     **/
    findSingleInputToBasketGivenBasketUnit(inputQuantity: TokenQuantity, rToken: Token, basketUnit: TokenQuantity[], internalTradeSlippage: bigint, signerAddress: Address, onResult: (result: {
        inputQuantity: TokenQuantity;
        firstTrade: SwapPath | null;
        trading: SwapPaths;
        minting: SwapPaths;
    }) => Promise<void>, abortSignal: AbortSignal): Promise<void>;
    createZapMintOption(tradeforPrecursorsInput: TokenQuantity, firstTrade: SwapPath | null, rToken: Token, internalTradeSlippage: bigint, signerAddress: Address, onResult: (result: {
        inputQuantity: TokenQuantity;
        firstTrade: SwapPath | null;
        trading: SwapPaths;
        minting: SwapPaths;
    }) => Promise<void>, abortSignal: AbortSignal, precursorTokens: Awaited<ReturnType<typeof findPrecursorTokenSet>>['rules']): Promise<void>;
    ...
    debugZapIntoToken(userInput: TokenQuantity, output: Token, signerAddress: Address, opts?: ToTransactionArgs & {
        maxHops?: number;
    }): Promise<{
        failed: {
            searchResult: import("./SearcherResult").BaseSearcherResult;
    ...
    findSingleInputTokenSwap(dynamicInput: boolean, input: TokenQuantity, output: Token, destination: Address, slippage: bigint, abort: AbortSignal, maxHops: number, internalOnly?: boolean): Promise<MultiChoicePath>;
    tokenPrices: Map<Token, TokenQuantity>;
    fairPrice(qty: TokenQuantity): Promise<TokenQuantity | null>;
    findSingleInputTokenSwap_(input: TokenQuantity, output: Token, destination: Address, slippage: bigint, abort: AbortSignal, maxHops: number, dynamicInput: boolean, onResult: (result: SwapPath) => Promise<void>, rejectRatio?: number, internalOnly?: boolean): Promise<void>;
