...
class Curve {
  ...
  async init(
    ...
    for (const pool of Object.values(this.constants.POOLS_DATA)) {
      ...
      if (pool.sCurveRewards_address && pool.sCurveRewards_abi != null) {
        await this.setContract(
          pool.sCurveRewards_address,
          pool.sCurveRewards_abi()
        )
      }
    }
    ...
    await this.setContract(
      this.constants.ALIASES.fee_distributor,
      feeDistributorABI
    )
  ...
  async setContract(
    address: string,
    abi: Promise<JsonFragment[]> | JsonFragment[]
  ) {
  ...
  fetchFactoryPools = async (useApi = true): Promise<void> => {
    if (useApi) {
      this.constants.FACTORY_POOLS_DATA = lowerCasePoolDataAddresses(
        await getFactoryPoolsDataFromApi.call(this, false)
      )
    } else {
      this.constants.FACTORY_POOLS_DATA = lowerCasePoolDataAddresses(
  ...
  fetchCryptoFactoryPools = async (useApi = true): Promise<void> => {
    if (useApi) {
      this.constants.CRYPTO_FACTORY_POOLS_DATA = lowerCasePoolDataAddresses(
        await getFactoryPoolsDataFromApi.call(this, true)
      )
    } else {
      this.constants.CRYPTO_FACTORY_POOLS_DATA = lowerCasePoolDataAddresses(
