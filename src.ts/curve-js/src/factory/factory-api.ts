...
export async function getFactoryPoolsDataFromApi(
  ...
  if (this.whitelist != null) {
    rawPoolList = rawPoolList.filter((pool) =>
      this.whitelist?.has(pool.address.toLowerCase())
    )
  }
      ...
      if (isCrypto) {
        ...
        if (basePoolId) {
          ...
          FACTORY_POOLS_DATA[pool.id] = {
            ...
            full_name: pool.name,
            symbol: pool.symbol,
            implementation: pool.implementationAddress,
...
async function setFactorySwapContracts(
  this: ICurve,
  ...
  if (isCrypto) {
    await Promise.all(
      rawPoolList.map(async (pool) => {
        const addr = pool.address

        this.contracts[addr] = {
          contract: new Contract(
            addr,
            await cryptoFactorySwapABI(),
            this.provider
          ),
    ...
    await Promise.all(
      rawPoolList.map(async (pool) => {
        ...
        this.contracts[addr] = {
          contract: new Contract(
            addr,
            await implementationABIDict[pool.implementationAddress](),
            this.provider
          ),
          multicallContract: new MulticallContract(
            addr,
...
async function setFactoryGaugeContracts(
  this: ICurve,
  rawPoolList: IPoolDataFromApi[]
) {
  await Promise.all(
