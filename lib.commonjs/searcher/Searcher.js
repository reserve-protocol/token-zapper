...
class Searcher {
    ...
    async findSingleInputToBasketGivenBasketUnit(inputQuantity, rToken, basketUnit, slippage) {
        ...
        await Promise.all(inputPrTrade.map(async ({ input, output }) => {
            ...
            const swaps = await this.findSingleInputTokenSwap(input, output, this.universe.config.addresses.executorAddress, slippage, 2, false);
            const trade = swaps[0];
            if (trade == null) {
    ...
    async findRTokenIntoSingleTokenZapViaRedeem(rTokenQuantity, output, signerAddress, slippage = 0.0) {
        ...
        const underlyingToOutputTrades = await Promise.all(unwrapTokenQtys
            ...
            .map(async (qty) => {
            const potentialSwaps = await this.findSingleInputTokenSwap(qty, outputToken, signerAddress, slippage, 3, true);
    ...
    async findTokenZapViaTrade(userInput, rToken, signerAddress, slippage = 0.0) {
        ...
        const paths = await this.externalQuoters(inputTokenQuantity, rToken, signerAddress, slippage, false).catch(() => []);
    ...
    async findSingleInputToRTokenZap_(userInput, rToken, signerAddress, slippage = 0.0) {
        ...
        const searcherResult = new SearcherResult_1.MintRTokenSearcherResult(this.universe, userInput, parts, signerAddress, rToken);
    ...
    async externalQuoters(input, output, destination, slippage, dynamicInput) {
        ...
        await Promise.all(aggregators.map((router) => router
            .swap(executorAddress, destination, input, output, slippage)
            .then((i) => out.push(i))
            .catch(() => null)));
        return out;
    }
    async internalQuoter(input, output, destination, slippage = 0.0, maxHops = 3) {
    ...
    async findSingleInputTokenSwap(input, output, destination, slippage = 0.0, maxHops = 3, dynamicInput = false) {
