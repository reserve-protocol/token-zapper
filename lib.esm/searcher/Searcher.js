...
import { RedeemZap, MintZap, ZapViaATrade, } from './SearcherResult';
...
export class Searcher {
    ...
    async findRTokenIntoSingleTokenZapViaRedeem__(rTokenQuantity, output, signerAddress, slippage, onResult, abortSignal, startTime) {
        ...
        const generateRedeemPlan = async (trades = []) => {
            ...
            return new RedeemZap(this.universe, rTokenQuantity, {
                full: outputSwap,
    ...
    async findTokenZapViaTrade(userInput, rToken, signerAddress, slippage, abortSignal, startTime = Date.now()) {
        ...
        return paths
            ...
            .slice(0, 2)
            .map((path) => new ZapViaATrade(this.universe, userInput, new SwapPaths(this.universe, [inputTokenQuantity], [path], path.outputs, path.outputValue, signerAddress), signerAddress, rToken, startTime, abortSignal));
    }
        ...
        try {
            await this.findSingleInputToBasketGivenBasketUnit(inputTokenQuantity, rToken, unitBasket, slippage, async (inputQuantityToBasketTokens) => {
                try {
                    ...
                    await onResult(new MintZap(this.universe, userInput, parts, signerAddress, rToken, startTime, abort));
                }
                catch (e) {
                    // console.log(e)
                    // console.log(e.stack)
