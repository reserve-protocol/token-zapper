...
class AerodromeRouterSwap extends (0, Action_1.Action)('Aerodrome') {
    ...
    async plan(planner, inputs, _, predicted) {
        ...
        planner.add(lib.swapExactTokensForTokens(inputs[0] ?? predicted[0].amount, minOut.amount, this.path.steps.map((step) => step.step.intoRouteStruct()), this.universe.execAddress.address, Date.now() + 1000 * 60 * 10), 'Aerodrome.swapExactTokensForTokens(' + this.path.toString() + ')');
