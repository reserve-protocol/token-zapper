...
import { Contract, LiteralValue, Planner, printPlan, } from '../tx-gen/Planner';
...
export class BaseSearcherResult {
    ...
    async createZapTransaction(options) {
        ...
        try {
            ...
            let dust = result.dust.map((qty, index) => this.potentialResidualTokens[index].from(qty));
            dust = dust.slice(0, dust.length - 1);
            dust = dust.filter((i) => i.amount !== 0n);
            if (options.returnDust === true) {
                for (const tok of dust) {
                    const balanceOfDust = plannerUtils.erc20.balanceOf(this.universe, this.planner, tok.token, this.universe.config.addresses.executorAddress);
                    plannerUtils.erc20.transfer(this.universe, this.planner, balanceOfDust, tok.token, this.signer);
                }
            }
            const finalParams = this.encodePayload(this.outputToken.from(result.amountOut), options);
            ...
            this.swaps = new SwapPaths(this.swaps.universe, this.swaps.inputs, this.swaps.swapPaths, updatedOutputs, values.reduce((a, b) => a.add(b[0] ?? this.universe.usd.zero), this.universe.usd.zero), this.swaps.destination);
            const estimate = result.gasUsed.toBigInt() + result.gasUsed.toBigInt() / 10n;
            const finalTx = this.encodeTx(this.encodeCall(options, params), estimate);
            return new ZapTransaction(this.universe, finalParams, finalTx, estimate, this.swaps.inputs[0], this.swaps.outputs, this.planner);
        }
        catch (e) {
            console.log(printPlan(this.planner, this.universe)
                .map((i) => '  ' + i)
                .join('\n'));
            throw e;
        }
    ...
    async toTransaction(options = {}) {
        ...
        for (const step of this.swaps.swapPaths[0].steps) {
            ...
            if (step.action.proceedsOptions === DestinationOptions.Callee) {
                const out = plannerUtils.erc20.balanceOf(this.universe, this.planner, step.outputs[0].token, this.universe.config.addresses.executorAddress);
                plannerUtils.planForwardERC20(this.universe, this.planner, step.outputs[0].token, out, this.signer);
            }
        }
...
export class BurnRTokenSearcherResult extends BaseSearcherResult {
    ...
    async toTransaction(options = {}) {
        ...
        if (outputTokenOnZapper) {
            const out = plannerUtils.erc20.balanceOf(this.universe, this.planner, this.outputToken, this.universe.config.addresses.executorAddress);
...
export class MintRTokenSearcherResult extends BaseSearcherResult {
    ...
    async toTransaction(options = {}) {
        await this.setupApprovals();
        const ethBalance = Contract.createContract(EthBalance__factory.connect(this.universe.config.addresses.ethBalanceOf.address, this.universe.provider));
        const zapperLib = Contract.createContract(ZapperExecutor__factory.connect(this.universe.config.addresses.executorAddress.address, this.universe.provider));
        const trades = new Map();
        for (const trade of this.parts.trading.swapPaths) {
            for (const step of trade.steps) {
                const inputsVal = new LiteralValue(ParamType.fromString('uint256'), defaultAbiCoder.encode(['uint256'], [step.inputs[0].amount]));
        ...
        if (totalUsedInMinting.get(this.inputToken).amount !== 0n) {
            trades.set(this.inputToken, plannerUtils.erc20.balanceOf(this.universe, this.planner, this.inputToken, this.universe.config.addresses.executorAddress));
        }
        for (const mintPath of this.parts.minting.swapPaths) {
            for (const step of mintPath.steps) {
                ...
                if (total.amount !== step.inputs[0].amount) {
                    ...
                    if (currentUsersLeft === 1) {
                        ...
                        actionInput = this.planner.add(zapperLib.fpMul(actionInput, fraction, ONE_Val), `${inputToken} * ${formatEther(fraction)}`, `frac_${step.outputs[0].token.symbol}`);
