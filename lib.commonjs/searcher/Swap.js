...
class SwapPaths {
    ...
    constructor(universe, inputs, swapPaths, outputs, outputValue, destination) {
        ...
        this.outputValue = outputValue;
        this.destination = destination;
        const byInputs = new base_1.DefaultMap(() => []);
        ...
        this.swapPaths = [...byInputs.values()].map((group) => {
            const inputs = Token_1.TokenAmounts.fromQuantities(group.map((i) => i.inputs).flat()).toTokenQuantities();
            const outputs = Token_1.TokenAmounts.fromQuantities(group.map((i) => i.outputs).flat()).toTokenQuantities();
            const outputValue = Token_1.TokenAmounts.fromQuantities(group.map((i) => i.outputValue).flat()).toTokenQuantities();
