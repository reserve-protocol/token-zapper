...
import { BaseSearcherResult, BurnRTokenSearcherResult, MintRTokenSearcherResult, TradeSearcherResult } from './SearcherResult';
...
export declare class Searcher<const SearcherUniverse extends UniverseWithERC20GasTokenDefined> {
    ...
    findRTokenIntoSingleTokenZapViaRedeem(rTokenQuantity: TokenQuantity, output: Token, signerAddress: Address, slippage: bigint, abortSignal: AbortSignal): Promise<BurnRTokenSearcherResult[]>;
    findRTokenIntoSingleTokenZapViaRedeem__(rTokenQuantity: TokenQuantity, output: Token, signerAddress: Address, slippage: bigint, onResult: (result: BurnRTokenSearcherResult) => Promise<void>, abortSignal: AbortSignal): Promise<void>;
    ...
    findTokenZapViaTrade(userInput: TokenQuantity, rToken: Token, signerAddress: Address, slippage: bigint, abortSignal: AbortSignal): Promise<TradeSearcherResult[]>;
    findSingleInputToRTokenZap(userInput: TokenQuantity, rToken: Token, signerAddress: Address, slippage: bigint): Promise<{
        quote: MintRTokenSearcherResult | TradeSearcherResult;
        cost: {
            units: bigint;
            txFee: TokenQuantity;
            txFeeUsd: TokenQuantity;
    ...
    externalQuoters(input: TokenQuantity, output: Token, destination: Address, dynamicInput: boolean, slippage: bigint, abort: AbortSignal): Promise<{
        venue: DexRouter;
        path: SwapPath;
    }[]>;
    externalQuoters_(abort: AbortSignal, input: TokenQuantity, output: Token, destination: Address, dynamicInput: boolean, slippage: bigint, onResult: (path: [DexRouter, SwapPath]) => Promise<void>): Promise<void>;
