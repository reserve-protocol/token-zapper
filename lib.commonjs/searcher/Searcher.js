...
class Searcher {
    ...
    constructor(universe) {
        ...
        this.externalQuoteCache = this.universe.createCache(async ({ input, output, slippage, abort, dynamicInput, onResult }) => {
            const out = [];
            await await this.universe.swaps(input, output, async (res) => {
                out.push(res);
                try {
                    await onResult(res);
                }
            ...
            return out;
        }, this.config.requoteTolerance, (inp) => `${inp.input}->${inp.output}:${inp.slippage}:${inp.destination}`);
        this.internalQuoerCache = this.universe.createCache(async ({ maxHops, input, output, destination, }) => {
            ...
            // }
            const res = await Promise.all(swapPlans.map(async (plan) => {
                try {
        ...
        }, this.config.requoteTolerance, (inp) => `${inp.input}->${inp.output}:${inp.maxHops}`);
    ...
    async findSingleInputToBasketGivenBasketUnit(inputQuantity, rToken, basketUnit, internalTradeSlippage, signerAddress, onResult, abortSignal) {
        ...
        await Promise.all(tradeforPrecursorsInputs.map(async (option) => {
            ...
            await this.createZapMintOption(tradeforPrecursorsInput, firstTrade, rToken, internalTradeSlippage, signerAddress, onResult, abortSignal, precursorTokens).catch(() => { });
        }));
    }
    async createZapMintOption(tradeforPrecursorsInput, firstTrade, rToken, internalTradeSlippage, signerAddress, onResult, abortSignal, precursorTokens) {
        ...
        await Promise.all(inputPrTrade.map(async ({ input, output }) => {
            ...
            try {
                const potentialSwaps = await Promise.all([
                    this.findSingleInputTokenSwap(true, input, output, this.universe.config.addresses.executorAddress, internalTradeSlippage, abortSignal, 1).catch(() => null),
                    this.findSingleInputTokenSwap(true, input, output, this.universe.config.addresses.executorAddress, internalTradeSlippage, abortSignal, 5, true).catch(() => null),
                ]);
            ...
            const potentialSwaps = new MultiChoicePath_1.MultiChoicePath(this.universe, options);
                ...
                const recourseOn = async (balances, parent, tradeAction) => {
                    ...
                    if (tradeAction.action) {
                        ...
                        try {
                            ...
                            console.log(tradeAction.inputAsFractionOfCurrentBalance.toString());
        ...
        const allOptions = await this.perf.measurePromise('generateAllPermutations', (0, MultiChoicePath_1.generateAllPermutations)(this, multiTrades, precursorSet, signerAddress), rToken.symbol);
        this.debugLog(`allOptions=${allOptions.length}`);
        const candidateChunks = [
            ...(0, MultiChoicePath_1.chunkifyIterable)(allOptions, this.maxConcurrency, abortSignal),
        ];
    ...
    async findRTokenIntoSingleTokenZapViaRedeem__(rTokenQuantity, output, signerAddress, slippage, onResult, abortSignal, startTime) {
        ...
        const tradeToPreferredOutputThenOutput = async (preferredOutput) => {
            ...
            const tradeForPreferredOut = (await Promise.all(unwrapTokenQtys.map(async (qty) => {
                ...
                const potentialSwaps = await this.findSingleInputTokenSwap(true, qty, preferredOutput, this.universe.config.addresses.executorAddress, this.universe.config.defaultInternalTradeSlippage, AbortSignal.timeout(this.universe.config.routerDeadline), 3, false).catch(() => null);
            ...
            return [
                ...
                await this.findSingleInputTokenSwap(true, outSum, output, signerAddress, this.universe.config.defaultInternalTradeSlippage, AbortSignal.timeout(this.universe.config.routerDeadline), 3),
            ];
        };
        ...
        if (permutableTrades.length === 0) {
            ...
            const allposibilities = await (0, MultiChoicePath_1.generateAllPermutations)(this, permutableTrades, new Set([outputToken]), signerAddress);
    ...
    async debugZapIntoToken(userInput, output, signerAddress, opts) {
        ...
        const controller = (0, MultiChoicePath_1.createConcurrentStreamingEvaluator)(this, toTxArgs);
        const errors = [];
        const onResult = async (path) => {
            const zap = new SearcherResult_1.ZapViaATrade(this, userInput, path.intoSwapPaths(this.universe), signerAddress, output, startTime, controller.abortController.signal);
            try {
        ...
        await this.findSingleInputTokenSwap_(userInput, output, signerAddress, slippage, controller.abortController.signal, opts?.maxHops ?? 3, false, onResult, 0.9, true);
        controller.finishedSearching();
        await controller.resultReadyPromise;
        try {
    ...
    async findSingleInputToRTokenZap_(userInput, rToken, signerAddress, slippage, onResult, abort, endPosition = rToken, startTime = Date.now()) {
        ...
        await this.findSingleInputToBasketGivenBasketUnit(inputTokenQuantity, rToken, unitBasket, slippage, signerAddress, async (inputQuantityToBasketTokens) => {
            ...
            if (endPosition !== rToken) {
                const lastSteps = await this.findSingleInputTokenSwap(true, tradingBalances.get(rToken), endPosition, signerAddress, slippage, AbortSignal.timeout(this.universe.config.routerDeadline), 4, true);
    ...
    async externalQuoters_(input, output, onResult, opts) {
        ...
        await Promise.all(out.map(async (res) => {
            try {
                await onResult(res);
            }
            catch (e) { }
    ...
    async findSingleInputTokenSwap(dynamicInput, input, output, destination, slippage, abort, maxHops, internalOnly) {
        const out = [];
        await this.findSingleInputTokenSwap_(input, output, destination, slippage, abort, maxHops, dynamicInput, async (path) => {
            out.push(path);
        }, 0.9, internalOnly).catch((e) => {
            console.log(e);
        });
        if (out.length === 0) {
    ...
    async fairPrice(qty) {
        ...
        if (out != null) {
            ...
            const unitPrice = qty.amount === qty.token.scale ? out : out.div(qtyAsToken);
            this.tokenPrices.set(qty.token, unitPrice);
        }
        return out;
    }
    async findSingleInputTokenSwap_(input, output, destination, slippage, abort, maxHops, dynamicInput, onResult, rejectRatio = 0.9, internalOnly = false) {
        ...
        await Promise.all([
            this.internalQuoter(input, output, destination, emitResult, maxHops).catch(() => { }),
            internalOnly
                ? Promise.resolve()
