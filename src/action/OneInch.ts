  ...
  async encode (): Promise<ContractCall> {
    ...
    return new ContractCall(
      ...
      BigInt(swap.tx.value),
            `1Inch Swap (${swap.protocols.flat(5).map((i: any) => i.name).join(',')})`
    )
  }

  toString () {
    return `OneInch(path=[${this.actionQuote.protocols.flat(5).map((i: any) => i.name).join(',')}])`
  }

  async quote (_: TokenQuantity[]): Promise<TokenQuantity[]> {
    return [this.output[0].quantityFromBigInt(
      BigInt(this.actionQuote.toTokenAmount)
    )]
  }
