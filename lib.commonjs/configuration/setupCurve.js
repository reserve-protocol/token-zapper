...
class CurveIntegration {
    ...
    ngCurvePools;
    venue;
    constructor(universe, curveApi, dex, curvePools, ngCurvePools) {
        this.universe = universe;
        this.curveApi = curveApi;
        this.dex = dex;
        this.curvePools = curvePools;
        this.ngCurvePools = ngCurvePools;
    ...
    static async load(universe, config) {
        ...
        const ngPoolList = await Promise.all(Object.entries(config.ngPools).map(async ([_, poolAddress]) => {
            return await (0, CurveStableSwapNG_1.setupCurveStableSwapNGPool)(universe, await universe.getToken(Address_1.Address.from(poolAddress)));
        }));
        ...
        const out = new CurveIntegration(universe, curveApi, dex, normalCurvePoolList, ngPools);
