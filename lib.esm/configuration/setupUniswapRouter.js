    ...
    async plan(planner, [input], destination) {
        for (const { route, inputAmount, outputAmount } of this.route.trade.swaps) {
            if (route.protocol === Protocol.V3) {
                const v3Route = route;
                input = await this.planV3Trade(planner, V3Trade.createUncheckedTrade({
                    route: v3Route,
...
export const setupUniswapRouter = async (universe) => {
    ...
    return {
        ...
        addTradeAction: (inputToken, outputToken) => {
            universe.addAction(new RouterAction(out, universe, routerAddr, inputToken, outputToken), routerAddr);
        },
    };
};
