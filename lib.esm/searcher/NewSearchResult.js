...
//     } catch (e) {
//       // console.log(
//       //   'Failing program:', this.inputToken.toString(), this.outputToken.toString(),
...
//           if (input.token === this.userInput.token) {
//             tradeInputs.set(
//               input.token,
...
//       executorAddress
//     )
//     for (const [token, qty] of tradeOutputs) {
//       const out = plannerUtils.erc20.balanceOf(
//         this.universe,
//         this.planner,
//         token,
...
//         encodeArg(this.userInput.amount, ParamType.from('uint256'))
//       )
//       const exchange = (step: SingleSwap, outputValues: Value[]) => {
//         for (const input of step.inputs) {
//           tradingBalances.sub(input)
//           if (mintingBalances.tokenBalances.has(input.token)) {
//             mintingBalances.sub(input)
//           }
//         }
//         for (let i = 0; i < step.outputs.length; i++) {
...
//           tradeBalanceValues.set(output.token, outputValue)
//           if (mintingBalances.tokenBalances.has(output.token)) {
//             mintingBalances.add(output)
//           }
//         }
//       }
...
//             }
//           }
//           for (const input of step.outputs) {
//             if (totalUsedInMinting.tokenBalances.has(input.token)) {
//               totalUsedInMinting.add(input)
//             }
//           }
//         }
//       }
//       for (const mint of this.parts.minting.swapPaths) {
//         for (const step of mint.steps) {
//           for (const input of step.inputs) {
//             if (!trades.has(input.token)) {
//               trades.set(
//                 input.token,
//                 plannerUtils.erc20.balanceOf(
//                   this.universe,
...
//   }
// }
//# sourceMappingURL=NewSearchResult.js.map