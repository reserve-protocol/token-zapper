...
describe('dag builder', () => {
  ...
  describe('mints', () => {
    it('1 WETH => STETH', async () => {
      ...
      //     (act) =>
      //       act.is1to1 &&
      //       (act.inputToken[0] === universe.nativeToken ||
      //         act.inputToken[0] === universe.wrappedNativeToken) &&
      //       act.outputToken[0] === steth
      //   )
      //   .map((i) => wrapAction(universe, i))

      // const builder = TokenFlowGraphBuilder.createSingleStep(
      //   universe.wrappedNativeToken.from(1.0),
      //   actions
      // )
      // console.log(builder.graph.toDot().join('\n'))

      // for (const node of builder.graph.sort()) {
