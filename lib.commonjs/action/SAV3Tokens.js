...
class MintSAV3TokensAction extends Action_1.Action {
    ...
    async plan(planner, inputs, destination, predicted) {
        const lib = this.gen.Contract.createContract(IStaticAV3TokenLM__factory_1.IStaticAV3TokenLM__factory.connect(this.outputToken[0].address.address, this.universe.provider));
...
class BurnSAV3TokensAction extends Action_1.Action {
    ...
    async plan(planner, inputs, destination, predicted) {
        const lib = this.gen.Contract.createContract(IStaticAV3TokenLM__factory_1.IStaticAV3TokenLM__factory.connect(this.inputToken[0].address.address, this.universe.provider));
    ...
    async quote([amountsIn]) {
        IStaticAV3TokenLM__factory_1.IStaticAV3TokenLM__factory.connect(this.inputToken[0].address.address, this.universe.provider);
        await this.universe.refresh(this.address);
        return [
            this.saToken
