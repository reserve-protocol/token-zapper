...
const initialize = async (universe) => {
    ...
    if (universe.chainConfig.config.curveConfig.enable) {
        const curvePools = await (0, Curve_1.loadCurvePools)(universe);
        await (0, Curve_1.addCurvePoolEdges)(universe, curvePools);
    }
    ...
    await defineRToken(universe, universe.rTokens.ETHPlus, Address_1.Address.from('0xDC690b50824D24F3421a27F4FA8EfCbCF1cAf986'));
};
