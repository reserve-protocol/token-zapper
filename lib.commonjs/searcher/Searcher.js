...
class Searcher {
    ...
    constructor(universe) {
        ...
        this.tokenPriceCache = universe.createCache(async (token) => await this.universe.fairPrice(token.one).then(() => universe.usd.zero), this.config.requoteTolerance);
    }
    /**
    ...
    async findSingleInputToBasketGivenBasketUnit(inputQuantity, rToken, basketUnit, internalTradeSlippage, onResult, abortSignal) {
         ...
         */
        const precursorTokens = await this.perf.measurePromise('findPrecursorTokenSet', (0, exports.findPrecursorTokenSet)(this.universe, inputQuantity, rToken, basketUnit, this), rToken.symbol);
        ...
        const allOptions = this.perf.measure('generateAllPermutations', () => generateAllPermutations(multiTrades));
    ...
    async externalQuoters_(abort, input, output, destination, dynamicInput, slippage, onResult) {
        ...
        await Promise.all(aggregators.map(async (router) => {
            try {
                const res = await this.perf.measurePromise(`external_routers_${router.name}`, router.swap(abort, executorAddress, destination, input, output, slippage), tradeName);
    ...
    async findSingleInputTokenSwap_(input, output, destination, slippage, abort, maxHops = 2, dynamicInput = false, onResult) {
        ...
        const emitResult = async (path) => {
            ...
            if (inValue != 0 && outValue != 0) {
                ...
                if (ratio < 0.95) {
                    // console.log(
                    //   `Dropping path ${path.inputs.join(
                    //     ', '
                    //   )}(${inValue}) -> ${path.outputs.join(
                    //     ', '
                    //   )} (${outValue}) due to low ratio: ${ratio}`
                    // )
                    dropped += 1;
                    return;
                }
        ...
        await Promise.all([
            ...
            this.externalQuoters_(abort, input, output, destination, dynamicInput, slippage, async ([_, path]) => {
                await emitResult(path);
            }),
        ]);
        ...
        // }
        // console.log(
        //   `(${inValue}) ${input} -> ${output}, dropped ${dropped} / ${total} paths`
        // )
    }
}
