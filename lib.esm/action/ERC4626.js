...
import { ETHTokenVault__factory, IStakedEthenaUSD__factory, IWrappedNative__factory, } from '../contracts';
...
export const ETHTokenVaultDepositAction = (proto) => class ETHTokenVaultDepositAction extends Action(proto) {
    universe;
    shareToken;
    vaultAddress;
    slippage;
    ...
    async plan(planner, inputs, destination, predicted) {
        ...
        const weth = this.gen.Contract.createContract(IWrappedNative__factory.connect(this.universe.wrappedNativeToken.address.address, this.universe.provider));
        planner.add(weth.withdraw(inputs[0]));
        const out = planner.add(lib.deposit(destination.address).withValue(inputs[0]), `${proto}.deposit{value:${predicted[0]}}(${destination.address})`);
        return [out];
    }
    gasEstimate() {
    ...
    constructor(universe, shareToken, vaultAddress, slippage) {
        super(shareToken.address, [universe.wrappedNativeToken], [shareToken], InteractionConvention.ApprovalRequired, DestinationOptions.Recipient, [new Approval(universe.wrappedNativeToken, shareToken.address)]);
        this.universe = universe;
        this.shareToken = shareToken;
        this.vaultAddress = vaultAddress;
        this.slippage = slippage;
        this.inst = ETHTokenVault__factory.connect(vaultAddress.address, this.universe.provider);
