...
class CurveSwap extends Action_1.Action {
    ...
    async _quote(amountsIn) {
        ...
        if (key in this.predefiendRoutes) {
            ...
            const output = (0, utils_1.formatUnits)(out.sub(out.div(10000n).mul(7n)), 18);
            return {
                output,
                route,
            };
        ...
        try {
            ...
            out.output = (0, utils_1.formatUnits)(outParsed.sub(outParsed.div(10000n).mul(7n)), 18);
            return out;
        }
        catch (e) {
            throw e;
    ...
    constructor(pool, tokenIn, tokenOut, predefiendRoutes) {
        ...
        this.predefiendRoutes = predefiendRoutes;
    }
    toString() {
...
const loadCurve = async (universe, predefinedRoutes) => {
    ...
    const loadCurvePools = async (universe) => {
        ...
        const poolsUnfiltered = poolNames.map((name) => {
            const pool = api_1.default.getPool(name);
            return { name, pool };
        });
