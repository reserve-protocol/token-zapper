...
import { _getCoinAddresses, _getCoinDecimals, _getUsdRate, isEth, parseUnits, ETH_ADDRESS, } from "./utils";
            ...
            for (const [poolId, poolData] of ALL_POOLS) {
                ...
                if (coin_addresses.length < 6 && inCoin === token_address) {
                    for (let j = 0; j < coin_addresses.length; j++) {
                        ...
                        const newRoutesByTvl = routesByTvl[inCoin].map((route) => getNewRoute(route, poolId, poolAddress, inCoin, coin_addresses[j], 0, j, swapType, AddressZero, tvl));
                        const newRoutesByLength = routesByLength[inCoin].map((route) => getNewRoute(route, poolId, poolAddress, inCoin, coin_addresses[j], 0, j, swapType, AddressZero, tvl));
                ...
                if (coin_addresses.length < 6 && inCoinIndex >= 0) {
                    ...
                    if (!(step === 3 && token_address !== outputCoinAddress)) {
                        ...
                        const newRoutesByTvl = routesByTvl[inCoin].map((route) => getNewRoute(route, poolId, poolAddress, inCoin, token_address, coin_addresses.indexOf(inCoin), 0, swapType, AddressZero, tvl));
                        const newRoutesByLength = routesByLength[inCoin].map((route) => getNewRoute(route, poolId, poolAddress, inCoin, token_address, coin_addresses.indexOf(inCoin), 0, swapType, AddressZero, tvl));
                ...
                if (inCoinIndexes.wrapped_coin >= 0 && !poolData.is_fake) {
                    for (let j = 0; j < wrapped_coin_addresses.length; j++) {
                        ...
                        const newRoutesByTvl = routesByTvl[inCoin].map((route) => getNewRoute(route, poolId, poolData.swap_address, inCoin, wrapped_coin_addresses[j], inCoinIndexes.wrapped_coin, j, swapType, AddressZero, tvl));
                        const newRoutesByLength = routesByLength[inCoin].map((route) => getNewRoute(route, poolId, poolData.swap_address, inCoin, wrapped_coin_addresses[j], inCoinIndexes.wrapped_coin, j, swapType, AddressZero, tvl));
                            ...
                            : (base_pool?.is_lending && poolData.is_factory) ? 5
                                : hasEth && poolId !== 'avaxcrypto' ? 3
                                    : poolData.is_crypto ? 4
                                        : 2;
                        const newRoutesByTvl = routesByTvl[inCoin].map((route) => getNewRoute(route, poolId, poolAddress, inCoin, underlying_coin_addresses[j], inCoinIndexes.underlying_coin, j, swapType, (swapType === 5 || swapType === 6) ? poolData.swap_address : AddressZero, tvl));
                        const newRoutesByLength = routesByLength[inCoin].map((route) => getNewRoute(route, poolId, poolAddress, inCoin, underlying_coin_addresses[j], inCoinIndexes.underlying_coin, j, swapType, (swapType === 5 || swapType === 6) ? poolData.swap_address : AddressZero, tvl));
...
const _getBestRoute = memoize(async (inputCoinAddress, outputCoinAddress, amount) => {
    ...
    if (_amount.eq(0))
        return [];
    const routesRaw = (await _findAllRoutes(inputCoinAddress, outputCoinAddress)).map((route) => ({ route, _output: ethersBigNumber.from(0), outputUsd: 0, txCostUsd: 0 }));
    const routes = [];
    try {
        const calls = [];
        ...
        for (const r of routesRaw) {
            const { _route, _swapParams, _factorySwapAddresses } = _getExchangeMultipleArgs(r.route);
            const p = contract.get_exchange_multiple_amount(_route, _swapParams, _amount, _factorySwapAddresses, curve.constantOptions);
    ...
    const expectedAmounts = (routes).map((route) => Number(formatUnits(route._output, outputCoinDecimals)));
