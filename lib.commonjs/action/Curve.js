...
class CurveSwap extends Action_1.Action {
    ...
    async plan(planner, inputs, _, [amountsIn]) {
        ...
        planner.add(curveRouterCallLib.exchange(inputs[0] ?? amountsIn.amount, minOut.amount - minOut.amount / 100n, routerContract.address, payload), `Curve,swap=${amountsIn} -> ${minOut}`, `amt_${this.output[0].symbol}`);
    ...
    async encode([amountsIn]) {
        ...
        return new ContractCall_1.ContractCall((0, utils_2.parseHexStringIntoBuffer)(data), exchangeAddress, value, this.gasEstimate(), `Swap ${amountsIn} for at least ${minOut} on Curve}`);
    }
    async _quote(amountsIn) {
        ...
        if (key in this.predefinedRoutes) {
            ...
            const output = (0, utils_1.formatUnits)(out.sub(out.div(600n)), this.output[0].decimals);
            return {
                output,
                route,
            };
        ...
        const task = (async () => {
            try {
                const out = await src_1.default.router.getBestRouteAndOutput(amountsIn.token.address.address, this.output[0].address.address, amountsIn.format());
                ...
                this.estimate = gasEstimate.toBigInt();
                const outParsed = (0, utils_1.parseUnits)(parseFloat(out.output).toFixed(this.output[0].decimals), this.output[0].decimals);
                out.output = (0, utils_1.formatUnits)(outParsed.sub(outParsed.div(600n)), this.output[0].decimals);
                return out;
            }
            catch (e) {
                throw e;
    ...
    async quote([amountsIn]) {
        ...
        const q = [this.output[0].fromScale18BN((0, utils_1.parseUnits)(out, 18))];
        return q;
    }
