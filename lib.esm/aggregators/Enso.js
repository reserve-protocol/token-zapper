...
const getEnsoQuote_ = async (abort, universe, quantityIn, tokenOut, recipient, slippage) => {
    ...
    const GET_QUOTE_DATA = `${API_ROOT}?chainId=${universe.chainId}&slippage=${slippage.toString()}&fromAddress=${execAddr}&routingStrategy=router&priceImpact=false&spender=${execAddr}`;
...
const getEnsoQuote = async (abort, universe, quantityIn, tokenOut, recipient, slippage, retries = 2) => {
    for (let i = 0; i < retries; i++) {
        ...
        try {
            return await getEnsoQuote_(abort, universe, quantityIn, tokenOut, recipient, slippage);
        }
        catch (e) {
            // console.log(
...
class EnsoAction extends Action('Enso') {
    ...
    async quote([input]) {
        if (Math.abs(this.lastQuoteBlock - this.universe.currentBlock) >
            this.universe.config.requoteTolerance) {
            try {
                this.request = await getEnsoQuote(AbortSignal.timeout(2000), this.universe, input, this.outputQty.token, this.address, this.slippage, 1);
            }
            catch (e) { }
        }
