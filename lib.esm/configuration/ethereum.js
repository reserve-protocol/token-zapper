...
import { IComptroller__factory, ICToken__factory, IStaticATokenLM__factory, } from '../contracts';
...
const initialize = async (universe) => {
    ...
    const chainLinkOracle = new ChainLinkOracle(universe, Address.fromHexString('0x47Fb2585D2C56Fe188D0E6ec628a38b74fCeeeDf'));
    ...
    const saUSDT = await universe.getToken(Address.fromHexString('0x21fe646d1ed0733336f2d4d9b2fe67790a6099d9'));
    const USDC = await universe.getToken(universe.config.addresses.commonTokens.USDC);
    const saUSDC = await universe.getToken(Address.fromHexString('0x60C384e226b120d93f3e0F4C502957b2B9C32B15'));
    const cEth = await universe.getToken(Address.from('0x4Ddc2D193948926D02f9B1fE9e1daa0718270ED5'));
    ...
    universe.defineMintable(new MintCTokenAction(universe, eth, cEth, cEthRate), new MintCTokenAction(universe, cEth, eth, cEthRate));
    const saTokens = [
    ...
    for (const { cToken, underlying } of cTokens) {
        ...
        universe.defineMintable(new MintCTokenAction(universe, underlying, cToken, rate), new BurnCTokenAction(universe, underlying, cToken, rate));
    }
    ...
    if (universe.config.addresses.rtokens.eUSD != null) {
        ...
        const basketHandler = new TokenBasket(universe, Address.fromHexString('0x6d309297ddDFeA104A6E89a132e2f05ce3828e07'), eUSD);
...
const ethereumConfig = {
    config: new StaticConfig({
        ...
        zapperAddress: Address.fromHexString('0xfa81b1a2f31786bfa680a9B603c63F25A2F9296b'),
        executorAddress: Address.fromHexString('0x7fA27033835d48ea32feB34Ab7a66d05bf38DE11'),
        rtokens: {
