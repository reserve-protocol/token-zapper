...
class BaseAaveV3 extends Action('SAV3Token') {
    get reserve() {
...
export class AaveV3Wrapper {
    ...
    static async create(aaveV3, saToken) {
        const wrapperInst = IStaticATokenV3LM__factory.connect(saToken.address.address, aaveV3.universe.provider);
        const aToken = await aaveV3.universe.getToken(Address.from(await wrapperInst.aToken()));
