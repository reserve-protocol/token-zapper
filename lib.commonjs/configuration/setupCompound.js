...
const ICToken_sol_1 = require("../contracts/factories/contracts/ICToken.sol");
...
async function setupCompoundLike(universe, deployment, cTokens) {
    ...
    if (cETH != null) {
        await (0, setupMintableWithRate_1.setupMintableWithRate)(universe, ICToken_sol_1.ICToken__factory, cETH, async (cEthRate, cInst) => {
            return {
    ...
    for (const { wrappedToken, underlying } of cTokens) {
        await (0, setupMintableWithRate_1.setupMintableWithRate)(universe, ICToken_sol_1.ICToken__factory, wrappedToken, async (rate, inst) => {
            return {
