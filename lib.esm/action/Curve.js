...
export const loadCurvePools = async (universe) => {
    ...
    const curvePools = await Promise.all(pools
        ...
        return new CurvePool(Address.from(pool.address), tokens, underlying, pool);
    }));
    return curvePools;
};
export const addCurvePoolEdges = async (universe, pools) => {
...
export class CurveSwap extends Action {
    ...
    constructor(pool, tokenInIdx, tokenIn, tokenOutIdx, tokenOut, exchangeUnderlying) {
        super(pool.address, [tokenIn], [tokenOut], InteractionConvention.ApprovalRequired, DestinationOptions.Callee, [
            new Approval(!exchangeUnderlying
