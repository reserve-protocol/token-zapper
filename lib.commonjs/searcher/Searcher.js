        ...
        await Promise.all(inputPrTrade.map(async ({ input, output }) => {
            ...
            for (let i = 1; i <= 4; i++) {
                try {
                    ...
                    const potentialSwaps = await this.findSingleInputTokenSwap(input, output, this.universe.config.addresses.executorAddress, internalTradeSlippage, abortSignal, i, true);
