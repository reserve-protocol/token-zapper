...
export const setupEthereumZapper = async (universe) => {
    ...
    await Promise.all(PROTOCOL_CONFIGS.erc4626.map(([addr, proto]) => setupERC4626(universe, [addr], proto)));
    ...
    const saV3Tokens = await convertWrapperTokenAddressesIntoWrapperTokenPairs(universe, PROTOCOL_CONFIGS.aavev3.tokenWrappers, aaveWrapperToUnderlying);
    await Promise.all(saV3Tokens.map(({ underlying, wrappedToken }) => setupSAV3Token(universe, wrappedToken, underlying)));
