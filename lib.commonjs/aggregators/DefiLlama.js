...
class DefillamaAction extends Action_1.Action {
    ...
    async encode(inputs, __) {
        return new ContractCall_1.ContractCall((0, utils_1.parseHexStringIntoBuffer)(this.request.rawQuote.data), Address_1.Address.from(this.request.rawQuote.to), 0n, this.gasEstimate(), `DefiLlama(${this.address}) (${inputs.join(',')}) -> (${await this.quote(inputs)})`);
    }
}
