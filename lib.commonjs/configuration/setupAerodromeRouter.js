...
class AerodromeRouterSwap extends (0, Action_1.Action)('Aerodrome') {
    ...
    async plan(planner, inputs, _, predicted) {
        const contract = contracts_1.IAerodromeRouter__factory.connect(this.router.address, this.universe.provider);
        const encodedSteps = this.path.steps.map((step) => step.step.intoRouteStruct());
        const encodedCall = contract.interface.encodeFunctionData('swapExactTokensForTokens', [
            predicted[0].amount,
            predicted[0].amount,
            encodedSteps,
        ...
        planner.add(this.universe.weirollZapperExecContract.rawCall(this.router.address, 0, encodedCall), this.toString());
        return null;
...
const setupAerodromeRouter = async (universe) => {
    ...
    const factories = new Map([
        [
            Address_1.Address.from('0x420DD381b31aEf6683db6B902084cB0FFECe40Da'),
            new AerodromeFactory(Address_1.Address.from('0x420DD381b31aEf6683db6B902084cB0FFECe40Da')),
        ],
        [
            Address_1.Address.from('0x5e7BB104d84c7CB9B682AaC2F3d509f5F406809A'),
            new AerodromeFactory(Address_1.Address.from('0x5e7BB104d84c7CB9B682AaC2F3d509f5F406809A')),
        ],
    ]);
    ...
    let pools = await Promise.all((await sugarInst.forSwaps(1000, 0)).map(async ({ token0, token1, lp, poolType, poolFee, factory }) => {
        ...
        const factoryAddr = Address_1.Address.from(factory);
        ...
        const pool = new AerodromePool(Address_1.Address.from(lp), tok0, tok1, poolToken, poolFee.toBigInt(), poolType, factoryInst);
        return pool;
    }));
    ...
    const out = new DexAggregator_1.DexRouter('aerodrome', async (abort, input, output, slippage) => {
        ...
        const outputs = [];
        await Promise.all(toEvaluate.slice(0, 40).map(async (route) => {
            try {
                if (outputs.length > 2) {
                    return null;
                }
