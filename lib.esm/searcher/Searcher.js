...
export class Searcher {
    ...
    async findRTokenIntoSingleTokenZapTx(rTokenQuantity, output, signerAddress, opts) {
        ...
        const mainPromise = Promise.all([
            this.findRTokenIntoSingleTokenZapViaRedeem__(rTokenQuantity, output, signerAddress, toTxArgs.internalTradeSlippage, controller.onResult, controller.abortController.signal).catch((e) => {
                // console.log(e)
                throw e;
            }),
    ...
    async findRTokenIntoSingleTokenZapViaRedeem__(rTokenQuantity, output, signerAddress, slippage, onResult, abortSignal) {
        ...
        const multiTrades = await Promise.all(unwrapTokenQtys
            ...
            .map(async (qty) => {
            const potentialSwaps = await this.findSingleInputTokenSwap(qty, outputToken, signerAddress, slippage, abortSignal, 1, true);
