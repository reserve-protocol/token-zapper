...
const promiseRaceWithin = async (promises, timeout) => {
    const waitingPromise = (0, controlflow_1.wait)(timeout);
    const out = [];
    const wrappedPromises = await Promise.all(promises.map((promise) => Promise.race([promise, waitingPromise.then((i) => null)]).catch(() => null)));
