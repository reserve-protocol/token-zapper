...
class Swap {
  ...
  toString () {
    return `Swap(input: ${this.input.map(i => i.formatWithSymbol()).join(', ')}, action: ${this.action}, output: ${this.output.map(i => i.formatWithSymbol()).join(', ')})`
  }
}
...
export class Swaps {
  ...
  toString () {
    return `Swaps(input: ${this.inputs.map(i => i.formatWithSymbol()).join(', ')}, steps: ${this.steps.join(', ')}, output: ${this.output.map(i => i.formatWithSymbol()).join(', ')} (${this.outputValue.formatWithSymbol()}))`
  }
}
...
export class SwapPlan {
  ...
  public async quote (
    ...
    const value = (await Promise.all(legAmount.map(async i => await this.universe.fairPrice(i).then(i => i ?? this.universe.usd.zero)))).reduce((l, r) => l.add(r))
