...
class ConvexStakingWrapper {
    ...
    async attachToUniverse() {
        ...
        for (const token of curvePool.allPoolTokens) {
            ...
            handlers.set(token, async (unit) => BasketTokenSourcingRuleApplication.singleBranch([unit.into(token)], [
                PostTradeAction.fromAction(await this.curve.findDepositAction(unit.into(token), curveLpToken), true),
