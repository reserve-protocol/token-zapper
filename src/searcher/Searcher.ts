...
export class Searcher {
  ...
  async findSingleInputToRTokenZap(params: ZapToRTokenParams) {
    ...
    const rTokenMintAction = (actions[0].output[0] === rToken ? actions[0] : actions[1]) as MintRTokenAction
    const baseTokenAmountsNeededPrRToken = new TokenAmounts()
    const baseTokenSplits = new DefaultMap<Token, Array<{ rate: TokenQuantity, action: Action }>>(() => ([]))
    const inputQuantitiesPrRToken = await rTokenMintAction.basketHandler.getMintQuantities()
    for (const qty of inputQuantitiesPrRToken) {
      ...
      if (acts != null) {
        ...
        baseTokenSplits.get(baseQtyNeeded.token).push({ rate: baseQtyNeeded, action: acts.mint })
      } else {
        baseTokenAmountsNeededPrRToken.add(qty)
        baseTokenSplits.get(qty.token).push({ rate: qty, action: rTokenMintAction })
      }
    }

    const tokensToBuy = [...baseTokenAmountsNeededPrRToken.tokenBalances.values()].filter(i => tradingBalances.get(i.token).amount === 0n).map(i => i.token)
    const swaps: Swaps[] = []
    const executorAddress = this.universe.config.addresses.executorAddress

    if (tokensToBuy.length !== 0) {
      const tradeInputAmount = inputTokenQuantity.scalarDiv(BigInt(tokensToBuy.length))

      const innerTrades = await Promise.all(tokensToBuy.map(async tokenWeNeedToBuy => {
        const swaps = await this.searchForTokenSwap(tradeInputAmount, tokenWeNeedToBuy, executorAddress)
    ...
    for (const baseTokenQuantity of baseTokenAmountsNeededPrRToken.tokenBalances.values()) {
      ...
      for (const split of baseTokenSplits.get(baseTokenQuantity.token)) {
        ...
        const mintAction = await new SwapPlan(this.universe, [split.action]).quote([mintInput], executorAddress)
        tradingBalances.exchange(mintAction.inputs, mintAction.output)
        swaps.push(mintAction)
      }
    }

    const rTokenInput = inputQuantitiesPrRToken.map(({ token }) => tradingBalances.get(token))
    const rTokenMint = await new SwapPlan(this.universe, [rTokenMintAction]).quote(rTokenInput, signerAddress)
    swaps.push(
      rTokenMint
    )
    tradingBalances.exchange(rTokenMint.inputs, rTokenMint.output)
    const output = [...tradingBalances.tokenBalances.values()].filter(i => i.amount !== 0n)
    output.sort((l, r) => -l.compare(r))

    const searcherResult = new SearcherResult(this.universe, this.approvals, [userInput], swaps, output, signerAddress)
    return searcherResult
  }

  async externalQuoters(input: TokenQuantity, output: Token, destination: Address): Promise<Swaps[]> {
    const executorAddress = this.universe.chainConfig.config.addresses.executorAddress
    return await Promise.all(this.universe.dexAggregators.map(async router => await router.swap(
      executorAddress, destination, input, output, 0
    )
    ))
  }

  async internalQuoter(input: TokenQuantity, output: Token, destination: Address): Promise<Swaps[]> {
    const bfsResult = bfs(this.universe, this.universe.graph, input.token, output, 2)
    const swapPlans = bfsResult.steps.map(i => i.convertToSingularPaths()).flat()
  ...
  async searchForTokenSwap(input: TokenQuantity, output: Token, destination: Address): Promise<Swaps[]> {
    const quotes = (await Promise.all([
      this.internalQuoter(input, output, destination),
