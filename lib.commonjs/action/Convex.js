...
class ConvexDepositAndStake extends (0, Action_1.Action)('Convex') {
    ...
    async plan(planner, inputs, destination) {
        const lib = this.gen.Contract.createContract(contracts_1.ConvexStakingWrapper__factory.connect(this.convexPool.stakedConvexDepositToken.address.address, this.universe.provider));
...
const setupConvexEdges = async (universe, stakedConvexToken, convex) => {
    const convexBooster = contracts_1.IBooster__factory.connect(convex.address, universe.provider);
    const stkCVXTokenInst = contracts_1.ConvexStakingWrapper__factory.connect(stakedConvexToken.address.address, universe.provider);
