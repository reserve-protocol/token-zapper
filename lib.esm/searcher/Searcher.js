...
export class Searcher {
    ...
    async *findSingleInputToBasketGivenBasketUnit(inputQuantity, rToken, basketUnit, slippage) {
        ...
        const nextPermutation = () => {
            const randTrade = tradesWithOptions[Math.floor(Math.random() * tradesWithOptions.length)];
            randTrade.increment();
        };
    ...
    async findTokenZapViaTrade(userInput, rToken, signerAddress, slippage = 0.0) {
        ...
        const paths = await this.externalQuoters(inputTokenQuantity, rToken, signerAddress, 0, false).catch(() => []);
        ...
        return paths
            .filter(async (path) => {
            if (slippageCheckValue.gt(inputValue) || // only check slippage if the value is large enough
    ...
    async findSingleInputToRTokenZapTx(userInput, rToken, signerAddress, slippage = 0.0, toTxArgs = {}) {
        ...
        const txes = await Promise.all(results.slice(0, 5).map(async (searchResult) => {
            try {
                return {
