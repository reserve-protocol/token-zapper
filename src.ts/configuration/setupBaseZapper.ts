...
export const setupBaseZapper = async (universe: BaseUniverse) => {
  ...
  const registry: OffchainOracleRegistry = new OffchainOracleRegistry("BaseOracles", async (token: Token) => {
  ...
  await setupStargateWrapper(
    universe,
    PROTOCOL_CONFIGS.stargate.wrappers,
    {}
  )
