...
class CurveSwap extends (0, Action_1.Action)('Curve') {
    ...
    tokenOut;
    predefinedRoutes;
    async plan(planner, inputs, _, [amountsIn]) {
        const output = await this._quote(amountsIn);
        const minOut = this.outputToken[0].fromScale18BN((0, utils_1.parseUnits)(output.output, 18));
        ...
        planner.add(curveRouterCallLib.exchange(inputs[0] ?? amountsIn.amount, minOut.amount - minOut.amount / 1000n, routerContract.address, payload), `Curve,swap=${amountsIn} -> ${minOut}`, `amt_${this.outputToken[0].symbol}`);
    ...
    async _quote(amountsIn) {
        const key = (this.inputToken[0].address +
            '.' +
            this.outputToken[0].address).toLowerCase();
        ...
        if (key in this.predefinedRoutes) {
            ...
            const output = (0, utils_1.formatUnits)(out.sub(out.div(600n)), this.outputToken[0].decimals);
            return {
                output,
                route,
            };
        ...
        const task = (async () => {
            try {
                ...
                out.output = (0, utils_1.formatUnits)(outParsed.sub(outParsed.div(600n)), this.outputToken[0].decimals);
                return out;
            }
            catch (e) {
                throw e;
    ...
    constructor(universe, pool, tokenIn, tokenOut, predefinedRoutes) {
...
const loadCurve = async (universe, predefinedRoutes_) => {
    const predefinedRoutes = Object.fromEntries(Object.entries(predefinedRoutes_).map(([key, route]) => [
        key,
        Promise.resolve(route),
    ]));
    const curvesEdges = new DefaultMap_1.DefaultMap(() => new Map());
    ...
    const defineCurveEdge = (pool, tokenIn, tokenOut) => {
        ...
        const swap = new CurveSwap(universe, pool, tokenIn, tokenOut, predefinedRoutes);
