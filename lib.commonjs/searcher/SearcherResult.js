        ...
        for (const path of tradesToGenerate) {
            for (const step of path.steps) {
                ...
                const input = path.inputs.map((t) => dynInput
                    ? Action_1.plannerUtils.erc20.balanceOf(this.universe, this.planner, t.token, executorAddress)
                    : (0, Planner_1.encodeArg)(t.amount -
                        (t.amount *
                            this.universe.config.defaultInternalTradeSlippage) /
                            constants_1.FEE_SCALE, utils_1.ParamType.from('uint256')));
                    ...
                    if (!qtyKnownAtCompileTime) {
                        if (needsToSplit) {
                            ...
                            if (supportsDynamicInput) {
                                ...
                                inputsVal = Action_1.plannerUtils.erc20.balanceOf(this.universe, this.planner, inputToken, this.universe.config.addresses.executorAddress);
                            }
                        }
                    }
