...
class SearcherResult {
    ...
    async simulate({ data, value, inputToken, gasLimit = 10000000 }) {
        const overrides = {
        ...
        return await (await fetch('https://worker-rapid-glitter-6517.mig2151.workers.dev/', {
            method: 'POST',
            headers: {
    ...
    async toTransaction(options = {}) {
        ...
        const totalValue = (await this.universe.fairPrice(outputTokenOutput))?.add(valueOfDust) ?? valueOfDust;
        ...
        if (outputIsRToken) {
            ...
            outputTokenOutput.token.from(outputTokenOutput.amount - 1000n), this.signer));
        }
        ...
        payload = {
            ...
            amountIn: this.swaps.inputs[0].amount,
            commands: builder.contractCalls.map((i) => i.encode()),
            amountOut: outputTokenOutput.amount - 10000000n,
            tokenOut: amountOut.token.address.address,
            tokensUsedByZap: dustTokens.map((i) => i.address.address),
        };
