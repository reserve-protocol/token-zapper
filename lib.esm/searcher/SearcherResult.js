...
export class ZapViaATrade extends BaseSearcherResult {
    async toTransaction(options = {}) {
        ...
        for (const step of this.swaps.swapPaths[0].steps) {
            await this.checkIfSearchIsAborted();
            const sendingToSigner = step.action.proceedsOptions === DestinationOptions.Recipient &&
            ...
            if (step.supportsDynamicInput) {
                ...
                if (inputs == null) {
                    inputs = step.action.inputToken.map((token) => plannerUtils.erc20.balanceOf(this.universe, this.planner, token, this.universe.execAddress));
                }
            }
            inputs =
                inputs ??
                    step.inputs.map((token) => encodeArg(token.amount, ParamType.from('uint256')));
        ...
        if (!fullyConsumed.has(this.outputToken)) {
            const out = plannerUtils.erc20.balanceOf(this.universe, this.planner, this.outputToken, this.universe.config.addresses.executorAddress, `Output token balance`, 'out_balance');
            ...
            for (const step of this.parts.outputMint.steps) {
                ...
                if (step.action.proceedsOptions === DestinationOptions.Callee) {
                    for (const actionOut of step.action.outputToken) {
                        ...
                        plannerUtils.erc20.transfer(this.universe, this.planner, plannerUtils.erc20.balanceOf(this.universe, this.planner, actionOut, this.universe.execAddress, `${this.outputToken} balance`), actionOut, this.signer);
