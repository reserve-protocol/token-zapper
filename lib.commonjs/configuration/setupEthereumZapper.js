...
const setupEthereumZapper = async (universe) => {
    ...
    // Set up RETH
    await (0, setupRETH_1.setupRETH)(universe, ethereum_1.PROTOCOL_CONFIGS.rocketPool);
    // Set up Lido
    ...
    universe.addPreferredRTokenInputToken(universe.rTokens['ETH+'], commonTokens.WETH);
    universe.addPreferredRTokenInputToken(universe.rTokens['dgnETH'], commonTokens.WETH);
    universe.addPreferredRTokenInputToken(universe.rTokens['ETH+'], universe.nativeToken);
    universe.addPreferredRTokenInputToken(universe.rTokens['dgnETH'], universe.nativeToken);
    universe.addPreferredRTokenInputToken(universe.rTokens.eUSD, commonTokens.USDC);
    universe.addPreferredRTokenInputToken(universe.rTokens.USD3, commonTokens.USDC);
    universe.addPreferredRTokenInputToken(universe.rTokens.hyUSD, commonTokens.USDC);
    const depositToETHX = new ((0, ERC4626_1.ETHTokenVaultDepositAction)('ETHX'))(universe, universe.commonTokens.ETHx, Address_1.Address.from('0xcf5EA1b38380f6aF39068375516Daf40Ed70D299'), 1n);
    universe.addAction(depositToETHX);
    universe.tokenTradeSpecialCases.set(universe.commonTokens.ETHx, async (input, dest) => {
        if (input.token === universe.wrappedNativeToken) {
            return await new Swap_1.SwapPlan(universe, [depositToETHX]).quote([input], dest);
        }
        return null;
    });
    universe.addSingleTokenPriceOracle({
        token: universe.commonTokens.ETHx,
        oracleAddress: Address_1.Address.from('0xC5f8c4aB091Be1A899214c0C3636ca33DcA0C547'),
