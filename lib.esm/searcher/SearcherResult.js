...
export class BaseSearcherResult {
    ...
    async simulateNoNode({ data, value }) {
        try {
            ...
            console.log('error:', e.message, 'Failing program:', printPlan(this.planner, this.universe)
                .map((i) => '  ' + i)
                .join('\n'));
        }
        // console.log(
...
export class BurnRTokenSearcherResult extends BaseSearcherResult {
    ...
    async toTransaction(options = {}) {
        ...
        for (const path of this.parts.tradesToOutput) {
            const input = plannerUtils.erc20.balanceOf(this.universe, this.planner, path.steps[0].inputs[0].token, executorAddress);
