...
export class CurveSwap extends Action('Curve') {
    ...
    async _quote(amountsIn) {
        ...
        if (key in this.predefinedRoutes) {
            ...
            const output = formatUnits(out.sub(out.div(600n)), this.outputToken[0].decimals);
            return {
                output,
                route,
            };
        ...
        const task = (async () => {
            try {
                ...
                out.output = formatUnits(outParsed.sub(outParsed.div(600n)), this.outputToken[0].decimals);
                return out;
            }
            catch (e) {
                throw e;
