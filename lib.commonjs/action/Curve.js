...
class CurveSwap extends (0, Action_1.Action)('Curve') {
    ...
    async plan(planner, inputs, _, [amountsIn]) {
        ...
        planner.add(curveRouterCallLib.exchange(inputs[0] ?? amountsIn.amount, minOut.amount - minOut.amount / this.slippage, routerContract.address, payload), `Curve,swap=${amountsIn} -> ${minOut}`, `amt_${this.outputToken[0].symbol}`);
...
const loadCurve = async (universe) => {
    ...
    const defineCurveEdge = (pool, tokenIn, tokenOut, slippage) => {
    ...
    return {
        ...
        createRouterEdge: (tokenA, tokenB, slippage) => {
