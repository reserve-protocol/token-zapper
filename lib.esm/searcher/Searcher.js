 ...
 */
export const findPrecursorTokenSet = async (universe, userInputQuantity, rToken, unitBasket) => {
...
export class Searcher {
    ...
    async findSingleInputToBasketGivenBasketUnit(inputQuantity, rToken, basketUnit, slippage) {
         ...
         */
        const precursorTokens = await findPrecursorTokenSet(this.universe, inputQuantity, rToken, basketUnit);
        /**
    ...
    async recursivelyUnwrapQty(qty) {
        ...
        return new SwapPaths(this.universe, [qty], swapPlans, output, outputQuotes.reduce((l, r) => l.add(r), this.universe.usd.zero), this.universe.config.addresses.executorAddress);
    }
        ...
        const outputSwap = new SwapPaths(this.universe, [rTokenQuantity], [
            ...redeemRTokenForUnderlying.swapPaths.map(i => {
                if (i.outputs.length === 1 && i.outputs[0].token === outputToken) {
                    return new SwapPath(i.inputs, i.steps, i.outputs, i.outputValue, signerAddress);
                }
                return i;
            }),
            ...underlyingToOutputTrade,
        ], tokenAmounts.toTokenQuantities(), outputValue, signerAddress);
        return new SearcherResult(this.universe, rTokenQuantity, outputSwap, signerAddress, outputToken);
    }
    findSingleInputToRTokenZap(userInput, rToken, signerAddress, slippage = 0.0) {
        ...
        //   slippage
        // )
        return retryLoop(() => this.findSingleInputToRTokenZap_(userInput, rToken, signerAddress, slippage), {
            maxRetries: 3,
            retryDelay: 500,
            async onRetry(...args) {
                console.log(...args);
    ...
    async findSingleInputToRTokenZap_(userInput, rToken, signerAddress, slippage = 0.0) {
        ...
        const searcherResult = new SearcherResult(this.universe, userInput, new SwapPaths(this.universe, [inputTokenQuantity], [...inputQuantityToBasketTokens.swapPaths, rTokenMint], output, rTokenMint.outputValue, signerAddress), signerAddress, rToken);
        return searcherResult;
    }
    async externalQuoters(input, output, destination, slippage) {
        if (this.universe.wrappedTokens.has(output) ||
    ...
    async internalQuoter(input, output, destination, slippage = 0.0, maxHops = 1) {
