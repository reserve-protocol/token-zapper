...
class SwapPaths {
    ...
    static fromPaths(universe, paths) {
        ...
        const inputs = TokenAmounts_1.TokenAmounts.fromQuantities(paths.map((i) => i.inputs).flat());
        const outputs = TokenAmounts_1.TokenAmounts.fromQuantities(paths.map((i) => i.outputs).flat());
...
class SwapPlan {
    ...
    async quote(input, destination) {
        ...
        const value = (await Promise.all(legAmount.map((i) => this.universe.fairPrice(i).then((i) => i ?? this.universe.usd.zero)))).reduce((l, r) => l.add(r));
