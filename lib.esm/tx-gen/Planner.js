...
const makeTokenUsersTracker = () => new DefaultMap((token) => {
    const state = {
        users: [],
        total: 0,
    };
...
export class StatefullPlanner {
    ...
    phases;
    userInput;
    balanceValues = new DefaultMap(() => encodeArg(0, ParamType.from('uint256')));
    predictedTokenBalances = new TokenAmounts();
    ...
    addInputQuantity(value) {
        ...
        this.balanceValues.set(value.token, encodeArg(this.predictedTokenBalances.get(value.token).amount, ParamType.from('uint256')));
    }
    ...
    tokenUsers = new DefaultMap(() => new Set());
    analyzePath(path) {
