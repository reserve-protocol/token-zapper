...
const SLIPPAGE = new sdk_core_1.Percent(100, 10000);
class UniswapRouterAction extends Action_1.Action {
    ...
    async plan(planner, [input], destination) {
        for (const { route, inputAmount, outputAmount } of this.route.trade.swaps) {
            if (route.protocol === router_sdk_1.Protocol.V3) {
                const v3Route = route;
                input = await this.planV3Trade(planner, v3_sdk_1.Trade.createUncheckedTrade({
                    route: v3Route,
...
const setupUniswapRouter = async (universe) => {
    ...
    const out = new DexAggregator_1.DexRouter('uniswap', async (src, dst, input, output, slippage) => {
        ...
        const outputAmt = output.from(BigInt(route.trade.minimumAmountOut(SLIPPAGE).quotient.toString()));
    ...
    return {
        ...
        addTradeAction: (inputToken, outputToken) => {
            universe.addAction(new RouterAction_1.RouterAction(out, universe, routerAddr, inputToken, outputToken), routerAddr);
        },
    };
};
