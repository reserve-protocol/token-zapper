...
export const initCurveOnEthereum = async (universe, convexBooster, lpActionSlippage) => {
    ...
    const makeStkConvexSourcingRule = (depositAndStake) => async (input, unitAmount) => {
        ...
        return BasketTokenSourcingRuleApplication.singleBranch([unitAmount.into(USDC)], [
            PostTradeAction.fromAction(await curveApi.createRouterEdge(unitAmount.into(USDC), lpTokenQty.token, lpActionSlippage), true // Cause the Zapper to recalculate the inputs of the mints for the next step
            ),
    ...
    const dex = new DexRouter('Eth.Curve', async (abort, input, output, slippage) => {
    ...
    return {
        ...
        venue: new TradingVenue(universe, dex, async (input, output) => {
            return new RouterAction(dex, universe, curveApi.routerAddress, input, output, universe.config.defaultInternalTradeSlippage);
        }),
    };
};
