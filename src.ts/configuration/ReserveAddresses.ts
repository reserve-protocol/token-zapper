...
const makeAddressMap = <const T extends Partial<Record<ChainId, string>>>(
  mappings: T
) => {
  return Object.fromEntries(
    Object.entries(mappings).map(([chainId, address]) => [
      parseInt(chainId),
      address ? Address.from(address) : null,
    ])
  ) as Record<ChainId, Address | null>
}
/**
 ...
 */
export const RSR_ADDRESS: AddressMap = makeAddressMap({
...
export const ETHPLUS_ADDRESS: AddressMap = makeAddressMap({
...
export const getAddressesForChain = (chainId: ChainId) => {
  ...
  type Fields = keyof typeof addresses

  const getFieldAsStrigOrZero = (key: Fields) =>
    addresses[key]?.address ?? constants.AddressZero
  const strGetters = new Proxy(
    {},
    {
      get: (_, key: Fields | 'config') => {
        if (key === 'config') return addresses
        return getFieldAsStrigOrZero(key)
      },
    }
  ) as { config: typeof addresses } & {
    [K in Fields]: string
  }
