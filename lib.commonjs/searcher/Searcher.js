...
class Searcher {
                ...
                const recourseOn = async (balances, parent, tradeAction) => {
                    ...
                    if (tradeAction.action) {
                        ...
                        try {
                            ...
                            console.error(tradeInputToTokenSet.map(i => i.describe().join("\n")).join(', '));
    ...
    async zapIntoRToken(userInput, rToken, userAddress, opts) {
        ...
        const tradeZap = this.findTokenZapViaTrade(userInput, rToken, userAddress, slippage, controller.onResult, controller.abortController.signal, start).catch((e) => {
            errors.push(e);
        });
