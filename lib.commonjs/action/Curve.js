...
class CurveSwap extends (0, Action_1.Action)('Curve') {
    ...
    async plan(planner, inputs, _, predicted) {
        ...
        planner.add(curveRouterCallLib.exchange(inputs[0] ?? predicted[0].amount, minOut, routerContract.address, payload), `Curve,swap=${predicted.join(', ')} -> ${quoteWithout}`, `amt_${this.outputToken[0].symbol}`);
