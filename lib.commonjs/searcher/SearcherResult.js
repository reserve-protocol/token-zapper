...
const needsZeroedOutFirst = new Set([
    Address_1.Address.from('0xdac17f958d2ee523a2206206994597c13d831ec7'),
]);
class Step {
    inputs;
    action;
...
class BaseSearcherResult {
    ...
    async simulate(opts) {
        ...
        if (process.env.DEBUG_SIMULATION) {
            this.searcher.debugLog((0, Planner_1.printPlan)(this.planner, this.universe).join('\n') + '\n\n\n');
        }
        let errorMsg = '';
    ...
    async simulateAndParse(options, data) {
        ...
        const currentBalances = TokenAmounts_1.TokenAmounts.fromQuantities(await Promise.all(this.potentialResidualTokens.map((token) => this.searcher.universe.approvalsStore.queryBalance(token, this.universe.config.addresses.executorAddress, this.searcher.universe))));
...
class ZapViaATrade extends BaseSearcherResult {
    async toTransaction(options = {}) {
        ...
        let inputs = [
            (0, Planner_1.encodeArg)(this.userInput.amount, utils_1.ParamType.from('uint256')),
        ];
        for (const step of this.swaps.swapPaths[0].steps) {
            await this.checkIfSearchIsAborted();
            const sendingToSigner = step.action.proceedsOptions === Action_1.DestinationOptions.Recipient &&
            ...
            if (step.supportsDynamicInput) {
                ...
                if (inputs == null) {
                    inputs = step.action.inputToken.map((token) => Action_1.plannerUtils.erc20.balanceOf(this.universe, this.planner, token, this.universe.execAddress));
                }
            }
            inputs =
                inputs ??
                    step.inputs.map((token) => (0, Planner_1.encodeArg)(token.amount, utils_1.ParamType.from('uint256')));
        ...
        if (!fullyConsumed.has(this.outputToken)) {
            const out = Action_1.plannerUtils.erc20.balanceOf(this.universe, this.planner, this.outputToken, this.universe.config.addresses.executorAddress, `Output token balance`, 'out_balance');
...
class RedeemZap extends BaseSearcherResult {
    ...
    async toTransaction(options = {}) {
        ...
        const outputs = await redeemStep.action.planWithOutput(this.universe, this.planner, [(0, Planner_1.encodeArg)(this.userInput.amount, utils_1.ParamType.fromString('uint256'))], this.universe.config.addresses.executorAddress, [this.userInput]);
            ...
            for (const step of unwrapBasketTokenPath.steps) {
                ...
                if (input == null) {
                    input = step.action.supportsDynamicInput
                        ? Action_1.plannerUtils.erc20.balanceOf(this.universe, this.planner, step.inputs[0].token, executorAddress)
                        : (0, Planner_1.encodeArg)(step.inputs[0].amount, utils_1.ParamType.from('uint256'));
                }
        ...
        for (const path of tradesToGenerate) {
            for (const step of path.steps) {
                ...
                if (input == null) {
                    input = dynInput
                        ? Action_1.plannerUtils.erc20.balanceOf(this.universe, this.planner, stepInput, executorAddress)
                        : (0, Planner_1.encodeArg)(stepInputQty.amount - stepInputQty.amount / 1000000n, utils_1.ParamType.from('uint256'));
                }
                await step.action.plan(this.planner, [input], this.signer, step.inputs);
            }
        }
        const bal = Action_1.plannerUtils.erc20.balanceOf(this.universe, this.planner, outToken, executorAddress);
        Action_1.plannerUtils.planForwardERC20(this.universe, this.planner, outToken, bal, this.signer);
...
class MintZap extends BaseSearcherResult {
    ...
    async toTransaction(options = {}) {
        ...
        try {
            ...
            const totalUsedInTrading = new TokenAmounts_1.TokenAmounts();
            const actionsUsingThisInputExcludingRTokenMint = new DefaultMap_1.DefaultMap(() => new Set());
            ...
            for (const paths of this.parts.full.swapPaths) {
                for (const step of paths.steps) {
                    for (const qty of step.inputs) {
                        ...
                        if (step.action instanceof RTokens_1.MintRTokenAction) {
            ...
            if (this.parts.setup != null) {
                ...
                let input = [
                    new Planner_1.LiteralValue(utils_1.ParamType.fromString('uint256'), utils_1.defaultAbiCoder.encode(['uint256'], [this.parts.setup.inputs[0].amount])),
                ];
                    ...
                    if (!qtyKnownAtCompileTime) {
                        if (needsToSplit) {
                            const dynValue = dynamicTradeInputSplits.get(inputToken) ??
                                Action_1.plannerUtils.erc20.balanceOf(this.universe, this.planner, inputToken, this.universe.config.addresses.executorAddress);
                            ...
                            if (supportsDynamicInput) {
                                ...
                                inputsVal = Action_1.plannerUtils.erc20.balanceOf(this.universe, this.planner, inputToken, this.universe.config.addresses.executorAddress, `Rest of ${inputToken} into ${step.action.toString()}`);
                            }
                        }
                    }
            ...
            for (const mintPath of this.parts.minting.swapPaths) {
                for (const step of mintPath.steps) {
                    if (step.action instanceof RTokens_1.MintRTokenAction) {
                        await step.action.plan(this.planner, step.inputs.map((i) => (0, Planner_1.encodeArg)(i.amount, utils_1.ParamType.from('uint256'))), this.universe.config.addresses.executorAddress, step.inputs);
                        for (const outputToken of step.action.outputToken) {
                            trades.set(outputToken, Action_1.plannerUtils.erc20.balanceOf(this.universe, this.planner, outputToken, this.universe.config.addresses.executorAddress));
                        }
                        continue;
                    }
                    const inputToken = step.action.inputToken[0];
                    let actionInput = (0, Planner_1.encodeArg)(step.inputs[0].amount, utils_1.ParamType.from('uint256'));
                    ...
                    if (!amountIsKnowStatically) {
                        if (!generateSplit) {
                            ...
                            const bnFact = (0, utils_1.parseEther)(fraction.toFixed(18)).toBigInt();
            ...
            for (const step of this.parts.outputMint.steps) {
                ...
                await step.action.plan(this.planner, step.inputs.map((i) => {
                    ...
                    if (out == null) {
                        if (step.action.supportsDynamicInput) {
                            out = Action_1.plannerUtils.erc20.balanceOf(this.universe, this.planner, i.token, thisAddr);
                        }
                        else {
                            out = (0, Planner_1.encodeArg)(i.amount, utils_1.ParamType.from('uint256'));
                        }
                    }
                    return out;
                ...
                if (step.action.proceedsOptions === Action_1.DestinationOptions.Callee) {
                    for (const actionOut of step.action.outputToken) {
                        ...
                        Action_1.plannerUtils.erc20.transfer(this.universe, this.planner, Action_1.plannerUtils.erc20.balanceOf(this.universe, this.planner, actionOut, this.universe.execAddress, `${this.outputToken} balance`), actionOut, this.signer);
            ...
            if (this.parts.outputMint.proceedsOptions === Action_1.DestinationOptions.Recipient) {
