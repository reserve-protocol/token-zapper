...
export class CurveSwap extends Action('Curve') {
    ...
    async plan(planner, inputs, _, predicted) {
        ...
        const routerContract = curveInner.contracts[curveInner.constants.ALIASES.registry_exchange]
            .contract;
        const curveRouterCallLib = this.gen.Contract.createLibrary(CurveRouterCall__factory.connect(this.universe.config.addresses.curveRouterCall.address, this.universe.provider));
        const { _route, _swapParams, _factorySwapAddresses } = _getExchangeMultipleArgs(output.route);
        const payload = parseHexStringIntoBuffer(defaultAbiCoder.encode(['address[9]', 'uint256[3][4]', 'address[4]'], [_route, _swapParams, _factorySwapAddresses]));
    ...
    async _quote(amountsIn) {
        const contract = curveInner.contracts[curveInner.constants.ALIASES.registry_exchange]
            .contract;
        try {
            const out = await curve.router.getBestRouteAndOutput(amountsIn.token.address.address, this.outputToken[0].address.address, amountsIn.format());
...
export const loadCurve = async (universe) => {
    const fakeRouterTemplate = {
        address: Address.from('0x99a58482bd75cbab83b27ec03ca68ff489b5788f'),
        name: 'curve-router',
    };
    const router = new CurvePool(Address.from(fakeRouterTemplate.address), universe.nativeToken, [], [], fakeRouterTemplate, 'router');
    ...
    const loadCurvePools = async (universe) => {
        ...
        const curvePools = await Promise.all(pools.map(async ({ name, pool }) => {
            try {
                ...
                return new CurvePool(Address.from(pool.address), lpToken, tokens, underlying, pool, name);
            }
            catch (e) {
                console.log(e);
                return null;
