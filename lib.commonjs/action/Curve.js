...
class CurveSwap extends (0, Action_1.Action)('Curve') {
    ...
    async plan(planner, inputs, _, [amountsIn]) {
        ...
        planner.add(curveRouterCallLib.exchange(inputs[0] ?? amountsIn.amount, minOut.amount - minOut.amount / 100n, routerContract.address, payload), `Curve,swap=${amountsIn} -> ${minOut}`, `amt_${this.outputToken[0].symbol}`);
