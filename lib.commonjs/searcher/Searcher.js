        ...
        await Promise.all(inputPrTrade.map(async ({ input, output }) => {
            ...
            try {
                const potentialSwaps = await Promise.all([
                    ...
                    this.findSingleInputTokenSwap(true, input, output, this.universe.config.addresses.executorAddress, internalTradeSlippage, abortSignal, 2, true).catch(() => null),
                    this.findSingleInputTokenSwap(true, input, output, this.universe.config.addresses.executorAddress, internalTradeSlippage, abortSignal, 3, true).catch(() => null),
                ]);
    ...
    async findSingleInputToRTokenZap_(userInput, rToken, signerAddress, slippage, onResult, abort, endPosition = rToken, startTime = Date.now()) {
        ...
        await this.findSingleInputToBasketGivenBasketUnit(inputTokenQuantity, rToken, unitBasket, slippage, signerAddress, async (inputQuantityToBasketTokens) => {
            ...
            if (endPosition !== rToken) {
                const lastSteps = await this.findSingleInputTokenSwap(true, tradingBalances.get(rToken), endPosition, signerAddress, slippage, abort, 3);
