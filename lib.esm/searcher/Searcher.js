...
export class Searcher {
    ...
    async redeem(rTokenQuantity, output, signerAddress, opts) {
        ...
        void Promise.all([
            ...
            opts?.enableTradeZaps === false ? Promise.resolve() : this.findTokenZapViaTrade(rTokenQuantity, output, signerAddress, toTxArgs.internalTradeSlippage, controller.onResult, controller.abortController.signal, start).catch(() => { }),
        ]).catch(() => { });
    ...
    async findSingleInputToRTokenZap_(userInput, rToken, signerAddress, slippage, onResult, abort, endPosition = rToken, startTime = Date.now()) {
        ...
        await this.findSingleInputToBasketGivenBasketUnit(inputTokenQuantity, rToken, unitBasket, slippage, async (inputQuantityToBasketTokens) => {
            ...
            const rTokenMint = await new SwapPlan(this.universe, [
                rTokenActions.mint,
            ]).quote(mintAction.inputToken.map((token) => tradingBalances.get(token)), endPosition !== rToken ? this.config.addresses.executorAddress : signerAddress);
            ...
            if (endPosition !== rToken) {
                ...
                for (const lastStep of lastSteps.paths) {
                    ...
                    const updatedMinting = new SwapPaths(this.universe, inputQuantityToBasketTokens.trading.outputs, [
                        ...inputQuantityToBasketTokens.minting.swapPaths,
                        rTokenMint,
                    ], rTokenMint.outputs, rTokenMint.outputValue, this.universe.execAddress);
