...
class DefillamaAction extends Action {
    ...
    async encode(inputs, __) {
        return new ContractCall(parseHexStringIntoBuffer(this.request.rawQuote.data), Address.from(this.request.rawQuote.to), 0n, this.gasEstimate(), `DefiLlama(${this.address}) (${inputs.join(',')}) -> (${await this.quote(inputs)})`);
    }
}
