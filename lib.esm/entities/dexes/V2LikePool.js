...
const computeV2PoolAddress = (factory, token0, token1) => {
    ...
    return Address.from(getCreate2Address(factory.address, keccak256(salt), INIT_CODE_HASH));
};
...
export const standardSwap = async (inputQty, action) => {
...
export const standardEncoding = async (inputQty, to, action) => {
    let amount0 = 0n;
    let amount1 = 0n;
    if (action.direction === '0->1') {
        if (inputQty.token === action.pool.token0) {
    ...
    return Buffer.from(standardPoolIface
    ...
    static createStandardV2Pool(factory, tokenA, tokenB, fee, poolAddress) {
        ...
        poolAddress =
            poolAddress == null
                ...
                : poolAddress;
        return new V2Pool(poolAddress, token0, token1, 0n, 0n, fee, standardSwap, standardEncoding);
    }
}
