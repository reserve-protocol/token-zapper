...
import { Address, Searcher, Universe, createKyberswap } from './index';
...
const run = async () => {
    ...
    const testUserAddr = Address.from(walletDeployer.address);
    {
            ...
            for (const rtoken of rtokensToMint) {
                try {
                    ...
                    const zapTx = await resMint.toTransaction({
                        returnDust: true,
                        outputSlippage: 100000000n,
                    });
                    console.log(`${input} -> ${rtoken}`);
                    console.log(`${zapTx.input} -> ${zapTx.output.join(', ')}`);
                }
                catch (e) {
                    console.log(`${input} -> ${rtoken}`);
                    console.log('Failed');
        ...
        //   universe.rTokens.hyUSD.from('100.0'),
        // ]) {
        //   for (const output of [
